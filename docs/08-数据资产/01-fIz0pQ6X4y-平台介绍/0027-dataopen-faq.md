...
数据资产平台权限设计为首次登陆，组织的主管理员默认拥有权限，需由组织管理员，在资产平台权限管理页面未其他人赋予对应的权限，才能有登陆权限。
...
## 调用数据资产平台的接口获取到的dataList是空的是什么情况？
