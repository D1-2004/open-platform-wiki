...
> **[!NOTE]**
>
> 本教程接入的子流程中配置的执行动作为官方连接器下的执行动作，如子流程需配置自建连接器下的执行动作，详情请参考[创建连接器](../02-iO2mVD3wB2-开发指南/0013-create-connector.md)、[添加触发事件](../02-iO2mVD3wB2-开发指南/0014-add-trigger-event-1.md)、[添加执行动作](../02-iO2mVD3wB2-开发指南/0015-add-execution-action-1.md)和[创建连接流](../02-iO2mVD3wB2-开发指南/0002-create-a-connection-flow-1.md)。
...
- 创建一个企业H5微应用并获取应用的AgentId，如何创建可参考[创建企业内部应用](../../01-应用开发/02-4a8AMF6u2A-服务端API/0856-create-an-h5-application-for-your-enterprise.md)。
...
> **[!NOTE]**
>
> 子流程如何创建及调试，详情请参考[创建连接流](../02-iO2mVD3wB2-开发指南/0002-create-a-connection-flow-1.md)。
...
2. 在入参映射设置界面中，将**上文节点出参**参数映射给**本节点入参**参数，单击**确定**并**保存**。
   ...
   - **需要获取的花名册字段信息**：选择**固定值**，并设置为**sys01-positionLevel**，更多字段信息，请参考[花名册自定义字段业务code](../../01-应用开发/02-4a8AMF6u2A-服务端API/0935-roster-custom-field-business-code.md)。
...
3. 在**公式编辑**界面，根据需求自定义表达式内容。
   ...
   > **[!NOTE]**
   >
   > 获取数组第一个元素，表达式如何使用，请参考[表达式](../02-iO2mVD3wB2-开发指南/0011-expression-overview.md)。
...
2. 设置**微应用在企业的agentId**，依次选择**设置 > 自定义**，填写获取的H5微应用AgentId值。
   ...
   > **[!NOTE]**
   >
   > AgentId如何获取，详情请参考[创建企业内部应用](../../01-应用开发/02-4a8AMF6u2A-服务端API/0856-create-an-h5-application-for-your-enterprise.md)。
