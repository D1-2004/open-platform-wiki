...
> Path: 连接平台 / 开发指南 / 开发参考 > 鉴权设置 > Token鉴权
...
自定义鉴权字段，例如：accessKey和accessSecret，单击**下一步**。
...
设置token请求接口，包括**设置Token获取接口**、**设置参数位置**、**Token失效判断。**
...
2. **设置参数位置**（可选）
   ...
   选择**URL查询参数**类型，填写参数字段，选择**鉴权字段**，通过直接获取鉴权字段获取值。
...
选择**HTTP Header**请求方式，添加**token**，选择**固定值**，填写**Token获取接口**返回的结果值。
...
3. 进行**鉴权验证**
   ...
   填写accessKey和accessSecret，单击**鉴权验证**，查看返回结果是否符合预期，最后单击**完成**。
...
自定义鉴权字段，例如：accessKey和accessSecret。
...
选择**HTTP Header**请求方式，添加携带的参数，选择**表达式**，添加携带参数的值，单击**下一步。**
...
- **固定值**：支持填写固定值。
- **鉴权字段**：引用鉴权字段中的accessKey或accessSecret。
- **表达式**：Authorization值为字符串“Bearer ”与请求**Token获取接口**后返回结果中Body的result（**$.Body.result**）的值拼在一起的字符串，表达式如何使用，详情请参考[表达式](0011-expression-overview.md)。
