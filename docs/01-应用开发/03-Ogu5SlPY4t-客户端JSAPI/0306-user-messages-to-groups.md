---
title: "用户身份发送卡片消息到群"
source_url: "https://open.dingtalk.com/document/development/user-messages-to-groups"
namespace: "development"
slug: "user-messages-to-groups"
group: "应用开发"
tab: "客户端JSAPI"
...
本文档提供以当前用户的身份将卡片消息发送到指定群的能力，可以通过调用sendMessageToGroup的JSAPI会唤起发送消息的确定弹窗，用户点击确定后就可以将卡片消息到指定群。
...
1. 用户身份发送卡片消息到群的JSAPI需依赖[dingtalk-jsapi](https://www.npmjs.com/package/dingtalk-jsapi)，请先升级到最新版本的[dingtalk-jsapi](https://www.npmjs.com/package/dingtalk-jsapi)版本。
...
import { sendMessageToGroup } from 'dingtalk-jsapi/plugin/coolAppSdk';
...
sendMessageToGroup({
      ...
      openConversationIdList: ["cideJmxxxxxVK9yXLIEg=="],
      sendCardRequest: {
        cardData: {
          cardParamMap: {
            title: "xxxxx",
...
| context.clientId | String | 是 | 应用标识。   - 企业内部应用，传Appkey。   **[!NOTE]**  如何获取Appkey，请参见[基础概念-AppKey](https://open.dingtalk.com/document/orgapp/basic-concepts)。   - 第三方企业应用，传SuiteKey。   **[!NOTE]**  如何获取Appkey，请参见[基础概念-SuiteKey](https://open.dingtalk.com/document/orgapp/basic-concepts)。 |
| context.corpId | String | 是 | 企业CorpId，用于校验群所属企业，仅能安装到该企业的内部群。  **[!NOTE]**  如何获取CorpId，请参见[基础概念-CorpId](https://open.dingtalk.com/document/orgapp/basic-concepts)。 |
...
| detail.result[].openConversationId | String | 发送消息的群Id。 |
| detail.result[].success | Boolean | 该群聊卡片消息是否发送成功。 |
...
酷应用接入流程更多详情请参考文档[群聊酷应用](https://open.dingtalk.com/document/orgapp/group-chat-cool-app-overview#cfb54a7056ygl)。
