---
title: "消息发送与接收类型"
source_url: "https://open.dingtalk.com/document/development/robot-message-type"
namespace: "development"
slug: "robot-message-type"
group: "应用开发"
tab: "服务端API"
breadcrumb: "即时通信 > 机器人 > 消息发送与接收类型"
doc_id: "vzHFldOC5l"
updated_at: "2026-05-15 18:23:29"
---

> Source: https://open.dingtalk.com/document/development/robot-message-type
> Path: 应用开发 / 服务端API / 即时通信 > 机器人 > 消息发送与接收类型
> Updated: 2026-05-15 18:23:29
...
> 如果上述消息类型均无法满足你的要求，可以使用[互动卡片](0775-overview-card.md)。
...
- [批量发送人与机器人会话中机器人消息](0712-chatbots-send-one-on-one-chat-messages-in-batches.md)
- [机器人发送群聊消息](0714-the-robot-sends-a-group-message.md)
...
详情参考[机器人发送群聊消息](0714-the-robot-sends-a-group-message.md)。
...
| 图片类型 | sampleImageMsg | ``` {   "photoURL": "xxxx" } ``` | **[!NOTE]**  photoURL可填写图片的完整URL路径，也可填写图片的mediaId。mediaId可通过[上传媒体文件](0644-upload-media-files.md)接口，获取`media_id`参数值。 |
...
| 语音类型 | sampleAudio | ``` {   "mediaId": "@IR_P********nFkfhsisbf4A",   "duration":"xxxxx" } ``` | 语音消息：   - **mediaId**：通过[上传媒体文件](0644-upload-media-files.md)接口，获取`media_id`参数值。  **[!NOTE]**  支持ogg、amr格式。 - **duration**：语音消息时长，单位毫秒。   image |
| 文件类型 | sampleFile | ``` {   "mediaId":"@lAz*********shRs5m2pRL",   "fileName":"表格.xlsx",   "fileType":"xlsx" } ``` | 文件消息：   - **mediaId**：通过[上传媒体文件](0644-upload-media-files.md)接口，获取`media_id`参数值。 - **fileName**：文件名称。 - **fileType**：文件类型。   **[!NOTE]**  文件类型，支持xlsx、pdf、zip、rar、doc、docx格式。  image |
| 视频类型 | sampleVideo | ``` {   "duration":"999",   "videoMediaId":"$iQEL**********AXNSs6QPPAAABhKN2L3EEzTcTB84BFM7iCAAJomlt",   "videoType":"mp4",   "picMediaId":"$igE**********tAFzQUABtoAIYQBpAtR3SICqrQ7ZX0nEJzAKxUDzwAAAYSjdi2cBM02vwcACAAJomltCgQ"             } ``` | 视频消息：   - **duration**：语音消息时长，单位秒。 - **videoMediaId**：通过[上传媒体文件](0644-upload-media-files.md)接口，获取`media_id`参数值。 - **videoType**：视频类型，支持mp4格式。 - **picMediaId**：视频封面图，通过[上传媒体文件](0644-upload-media-files.md)接口，获取`media_id`参数值。 - **height**：视频展示高度，单位px。 - **width**：视频展示宽度，单位px。 |
...
| atMobiles | 否 | Array | 被@人的手机号。  **[!NOTE]**  在content里添加@人的手机号，且只有在群内的成员才可被@，非群内成员手机号会被脱敏。 |
| atUserIds | 否 | Array | 被@人的用户userid。  **[!NOTE]**  在content里添加@人的userId。 |
...
| messageUrl | String | 是 | 点击消息跳转的URL，打开方式如下：   - 移动端，在钉钉客户端内打开 - PC端    - 默认侧边栏打开   - 希望在外部浏览器打开，参考[消息链接说明](0774-message-link-description.md)。 |
...
| atMobiles | 否 | Array | 被@人的手机号。  **[!NOTE]**  在text内容里要有@人的手机号，只有在群内的成员才可被@，非群内成员手机号会被脱敏。 |
| atUserIds | 否 | Array | 被@人的用户userid。  **[!NOTE]**  在 text 中添加@人的userId。 |
| isAtAll | 否 | Boolean | @所有人是true，否则为false。 |
...
- **整体跳转ActionCard类型**
  ...
  | title | 是 | String | 首屏会话透出的展示内容。 |
  | text | 是 | String | markdown格式的消息。  **[!NOTE]**  如果需要实现 @ 功能 ，在 text 内容中添加 @ 用户的 userId。例如：@manager7675 |
  | singleTitle | 是 | String | 单个按钮的标题。 |
  | singleURL | 是 | String | 点击消息跳转的URL，打开方式如下：  - 移动端，在钉钉客户端内打开 - PC端    - 默认侧边栏打开   - 希望在外部浏览器打开，参考[消息链接说明](0774-message-link-description.md)。 |
...
- **独立跳转ActionCard类型**
  ...
  | title | 是 | String | 首屏会话透出的展示内容。 |
  | text | 是 | String | markdown格式的消息内容。  **[!NOTE]**  如果需要实现 @ 功能 ，在 text 内容中添加 @ 用户的 userId。例如：@manager7675 |
  | btns | 是 | Array | 按钮。 |
  | title | 是 | String | 按钮标题。 |
  | actionURL | 是 | String | 击按钮触发的URL，打开方式如下：  - 移动端，在钉钉客户端内打开 - PC端    - 默认侧边栏打开   - 希望在外部浏览器打开，参考[消息链接说明](0774-message-link-description.md)。 |
...
| messageURL | 是 | String | 点击单条信息到跳转链接，详情参考[消息链接说明](0774-message-link-description.md)。 |
...
```
{
  ...
  "isInAtList": true,
  "sessionWebhook": "https://oapi.dingtalk.com/robot/sendBySession?session=66d7c695a9xxxxdeb885",
  "text": {
    "content": " text"
  },
...
| content | Object | 消息内容。 |
| richText | Array of Object | 富文本列表。  **[!NOTE]**  消息列表中可以包含：   - **text：**文本消息 - **picture**：图片消息  图片文件的下载码downloadCode，可通过调用服务端API-[下载机器人接收消息的文件内容](0717-download-the-file-content-of-the-robot-receiving-message.md)接口获取临时下载链接。 |
...
| downloadCode | String | 图片文件的下载码，用于换取下载图片的二进制文件，可通过调用服务端API-[下载机器人接收消息的文件内容](0717-download-the-file-content-of-the-robot-receiving-message.md)接口获取临时下载链接。 |
...
| downloadCode | String | 语音文件的下载码，用于换取下载语音的二进制文件，可通过调用服务端API-[下载机器人接收消息的文件内容](0717-download-the-file-content-of-the-robot-receiving-message.md)口获取临时下载链接。 |
...
| downloadCode | String | 视频文件的下载码，用于换取下载视频的二进制文件，可通过调用服务端API-[下载机器人接收消息的文件内容](0717-download-the-file-content-of-the-robot-receiving-message.md)接口获取临时下载链接。 |
...
| downloadCode | String | 文件的下载码，用于换取下载文件的二进制文件，可通过调用服务端API-[下载机器人接收消息的文件内容](0717-download-the-file-content-of-the-robot-receiving-message.md)接口获取临时下载链接。 |
| fileName | String | 文件名。 |
...
| appSecret | 应用的 Client Secret，详情参考[Client Secret](../01-XOnnmGCTbn-开发指南/0001-basic-concepts-beta.md#section-pje-9wf-l7c)。 |
...
| --- | --- | --- | --- |
| 20001 | 受调用量超量影响，当前我的消息服务已经暂停。请联系你所在的组织管理员并予以处理  接收消息中，新增 **errorMessage** 字段，用于展示错误信息。 | - 接收消息中无 text 和 content 字段内容 - 登录[开发者后台](https://open-dev.dingtalk.com/)即可查看调用量。  image | 需要升级钉钉专业版或购买增购包，详情参看[企业增购包购买说明](https://open.dingtalk.com/document/orgapp/purchase-instructions-for-enterprise-additional-purchase-package)。 |

### **相关文档**

- [企业机器人发送群聊消息](https://open.dingtalk.com/document/dingstart/the-application-robot-in-the-enterprise-sends-group-chat-messages)
- [企业机器人发送单聊消息](https://open.dingtalk.com/document/dingstart/the-application-robot-in-the-enterprise-sends-a-single-chat)
