...
2. 调用服务端API-[客户资料](1054-add-or-edit-customer-profile.md)接口，进行客户信息的新增和编辑。
3. 调用服务端API-[产品信息](1064-add-or-edit-product-information.md)接口，进行产品信息的新增和编辑。
4. 调用服务端API-[销售机会](1060-add-or-edit-opportunities.md)接口，进行销售机会的新增和编辑。
5. 调用服务端API-[报价记录](1061-add-or-edit-quotation-records.md)接口，进行报价记录的新增和编辑。
6. 调用服务端API-[合同订单](1057-add-or-edit-contract-orders.md)接口，进行合同订单的新增和编辑。
7. 调用服务端API-[采购单](1062-edit-purchase-order.md)接口，进行采购单的新增和编辑。
8. 调用服务端API-[入库单](1065-add-or-edit-a-shipment-record.md)接口，进行入库单的新增和编辑。
9. 调用服务端API-[发货单](1058-add-or-edit-invoices.md)接口，进行发货单的新增和编辑。
10. 调用服务端API-[出库单](1066-add-or-edit-an-issue-ticket.md)接口，进行出库单的新增和编辑。

数据信息管理：

- 调用服务端API-[获取数据详情](1067-queries-data-details.md)接口，可以获取销售机会、报价单、订单、采购、入库、发货和出库一系列环节的数据详细信息。
- 调用服务端API-[获取数据列表](1068-obtain-the-data-list.md)接口，可得到某一单据的数据列表信息。
...
- 根据订单交付流程中msgId参数和dataType参数，调用服务端API-[获取数据详情](1067-queries-data-details.md)接口，可以获取销售机会、报价单、订单、采购、入库、发货和出库一系列环节的数据详细信息。
- 调用服务端API-[获取数据列表](1068-obtain-the-data-list.md)接口，可得到某一单据的数据列表信息。
