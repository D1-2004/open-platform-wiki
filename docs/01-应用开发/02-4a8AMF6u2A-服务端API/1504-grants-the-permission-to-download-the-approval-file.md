...
>
> - 如果未使用本接口，推荐使用新版规范[下载审批附件](https://open.dingtalk.com/document/orgapp/download-an-approval-attachment)接口。
...
>
>   需要与[获取单个审批实例详情](https://open.dingtalk.com/document/orgapp/obtains-the-details-of-a-single-approval-instance)接口结合使用，获取实例详情接口得到附件的fileName和fileType，按照本接口返回的`#zifgs49xxxxx.file`进行名称和后缀替换，该附件就可以正常打开了。
> - 附件文件大小不能为0，比如txt文件等，不支持获取下载链接。
> - 该接口只能下载审批附件钉盘空间的文件，无法下载到审批评论的附件。

调用本接口，可以下载审批附件钉盘空间的文件。![附件下载](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8294796361/p352488.png)
...
| process\_instance\_id | String | 是 | 123a-234bxx | 审批单实例id，调用[获取单个审批实例详情](https://open.dingtalk.com/document/orgapp/obtains-the-details-of-a-single-approval-instance)接口获取。 |
| file\_id | String | 是 | 123456 | 文件id，调用[获取单个审批实例详情](https://open.dingtalk.com/document/orgapp/obtains-the-details-of-a-single-approval-instance)接口获取。  **[!IMPORTANT]**  文件id是审批组件中上传的fileid（如下图所示），评论中上传的附件fileid暂不支持获取下载链接。  文件组件 |
