...
slug: "obtain-the-access-token-of-the-third-party-application-authorization-enterprise"
group: "应用开发"
tab: "服务端API"
breadcrumb: "认证与授权 > 访问凭证 > 应用身份凭证 > 定制应用 > 获取定制应用的accessToken"
doc_id: "ipM7ndFTqR"
...
> Path: 应用开发 / 服务端API / 认证与授权 > 访问凭证 > 应用身份凭证 > 定制应用 > 获取定制应用的accessToken
> Updated: 2026-04-29 22:27:43

# 获取定制应用的accessToken

产品服务商可通过此接口获取授权企业的accessToken。定制应用调用服务端API获取应用资源时，使用获取的accessToken进行身份验证。

## **请求**

| **基本信息** | |
| --- | --- |
| HTTP URL | https://api.dingtalk.com/v1.0/oauth2/corpAccessToken |
| HTTP Method | POST |
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| suiteKey | String | 是 | suitep1f5lzyglm7fryxxxx | 定制应用的CustomKey。 |
| suiteSecret | String | 是 | \_FP5PpZF3irDKjxxx | 定制应用的CustomSecret。 |
| authCorpId | String | 是 | ding123456 | 授权企业的CorpId。iShot2022-10-17 10 |
| suiteTicket | String | 是 | 1f5lzyglm7fryxxxx | 钉钉推送的suiteTicket，定制应用该参数自定义，比如Test。 |
...
```
curl -X POST 'https://api.dingtalk.com/v1.0/oauth2/corpAccessToken' \
...
| 名称 | 类型 | 示例值 | 描述 |
| --- | --- | --- | --- |
| accessToken | String | 1cc1bb3xxxx | 定制应用的的accessToken。  **[!NOTE]**   - 开发者需要缓存accessToken，用于后续接口的调用。因为每个应用的accessToken是彼此独立的，所以进行缓存时需要区分应用来进行存储。 - 不能频繁调用gettoken接口，否则会受到频率拦截。 |
| expireIn | Long | 7200 | 定制应用的accessToken超时时间，单位秒。  **[!NOTE]**  accessToken的有效期为7200秒（2小时），有效期内重复获取会返回新的accessToken。 |
