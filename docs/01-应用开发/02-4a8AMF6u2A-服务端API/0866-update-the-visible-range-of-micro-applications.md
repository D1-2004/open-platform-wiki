...
group: "应用开发"
tab: "服务端API"
breadcrumb: "应用管理 > 更新企业内部应用的可使用范围"
doc_id: "XiNPty8ODR"
...
> Path: 应用开发 / 服务端API / 应用管理 > 更新企业内部应用的可使用范围
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| addUserIds | Array of String | 否 | 待添加的用户userId列表，最大长度100。  **[!NOTE]**    添加后总用户数不得超过2000，否则将返回错误。 |
| delUserIds | Array of String | 否 | 删除的可使用用户userId列表，最大长度100。 |
| addDeptIds | Array of Long | 否 | 待添加的部门ID列表，最大长度100。  **[!NOTE]**    添加后总部门数不得超过2000，否则将返回错误。 |
| delDeptIds | Array of Long | 否 | 待删除的部门ID列表，最大长度100。 |
| addRoleIds | Array of Long | 否 | 待添加的角色ID列表，最大长度100。可通过[获取角色列表](0089-obtains-a-list-of-enterprise-roles.md)接口获取具体ID值。  **[!NOTE]**    添加后总角色数不得超过2000，否则接口会报错。 |
