---
title: "新增或更新表单实例"
source_url: "https://open.dingtalk.com/document/development/add-or-update-form-instances"
namespace: "development"
slug: "add-or-update-form-instances"
group: "应用开发"
tab: "服务端API"
breadcrumb: "历史文档（不推荐） > 宜搭 > 表单 > 新增或更新表单实例"
doc_id: "vZnhsJLHQx"
updated_at: "2025-09-08 19:03:42"
---

> Source: https://open.dingtalk.com/document/development/add-or-update-form-instances
> Path: 应用开发 / 服务端API / 历史文档（不推荐） > 宜搭 > 表单 > 新增或更新表单实例
> Updated: 2025-09-08 19:03:42
...
>
> - 如果未使用本接口，推荐使用[新增或更新表单实例](https://open.dingtalk.com/document/orgapp/api-createorupdateformdata-v2)接口。
> - 如果已使用本接口，建议您根据自身实际情况评估是否切换至推荐接口。

### 调用说明

企业宜搭应用的数据管理表单页面，如下图所示。
![](https://img.alicdn.com/imgextra/i4/O1CN01xMec8D1uCVXeDmeNB_!!6000000006001-2-tps-1530-922.png)

- 存在满足筛选条件的表单实例：
  **筛选条件：发起人是张某；表单状态为STRING；表单组件包含报销项目并指定该组件值为测试。**

表单数据列表中已存在的四个表单：发起人都是张某；状态都是STRING；都存在报销项目组件。 调用效果如下图所示，已根据指定表单值更新符合查询条件的表单实例。
![](https://img.alicdn.com/imgextra/i3/O1CN01qVNtmA1Ffuixq1kWM_!!6000000000515-2-tps-1536-916.png)

- 不存在满足筛选条件的表单实例：

  **筛选条件：发起人是王某；表单状态为STRING；表单组件包含报销项目并指定该组件值为测试。**表单数据列表中已存在的四个表单：发起人都是张某，不存在王某发起的表单。

调用效果如下图所示，已新增一条以张某为发起人的表单实例。

![](https://img.alicdn.com/imgextra/i3/O1CN01m1BxAA1EODoIjggKd_!!6000000000341-2-tps-1536-940.png)
...
```
POST /v1.0/yida/forms/instances/insertOrUpdate HTTP/1.1
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| noExecuteExpression | Boolean | 否 | 是否不要触发表单绑定的校验规则、关联业务规则和第三方服务回调。   - true：不触发。 - false：触发   **[!NOTE]**    该参数值传true可以降低API的耗时。 |
| formUuid | String | 是 | 表单编码。  **[!NOTE]**    该参数从宜搭应用中获取。 |
| searchCondition | String | 是 | 用于检索表单实例数据的检索条件。   - 企业内部应用，格式参考[宜搭表单筛选的组件格式说明](https://open.dingtalk.com/document/orgapp/use-the-filter-conditions-in-the-form-data-management-for)。 - 第三方企业应用，格式参考[宜搭表单筛选的组件格式说明](https://open.dingtalk.com/document/isvapp/description-of-component-formats-suitable-for-form-filtering)。 |
| appType | String | 是 | 宜搭应用编码。  **[!NOTE]**    该参数从宜搭应用中获取。 |
| formDataJson | String | 是 | 用于更新或新增表单实例的数据。   - 企业内部应用，格式参考[创建或更新表单数据格式说明](https://open.dingtalk.com/document/orgapp/instructions-for-creating-or-updating-form-data-format)。 - 第三方企业应用，格式参考[创建或更新表单数据格式说明](https://open.dingtalk.com/document/isvapp/instructions-for-creating-or-updating-form-data-format)。 |
| systemToken | String | 是 | 宜搭应用密钥。  **[!NOTE]**    该参数从宜搭应用中获取。 |
| userId | String | 是 | 用户userid。  **[!NOTE]**    该用户userid对应的用户需拥有该表单所有的数据管理权限，否则可能会导致无法更新数据。 |
...
public class Sample {
    ...
    public static void main(String[] args_) throws Exception {
        ...
        CreateOrUpdateFormDataRequest createOrUpdateFormDataRequest = new CreateOrUpdateFormDataRequest()
                ...
                .setFormDataJson("{\"countrySelectField_l0c1cwiu\":[{\"value\":\"US\"}]}")
...
func _main (args []*string) (_err error) {
  ...
  createOrUpdateFormDataHeaders := &dingtalkyida_1_0.CreateOrUpdateFormDataHeaders{}
  createOrUpdateFormDataHeaders.XAcsDingtalkAccessToken = tea.String("<your access token>")
  createOrUpdateFormDataRequest := &dingtalkyida_1_0.CreateOrUpdateFormDataRequest{
...
export default class Client {
  ...
  static async main(args: string[]): Promise<void> {
    ...
    createOrUpdateFormDataHeaders.xAcsDingtalkAccessToken = "<your access token>";
    let createOrUpdateFormDataRequest = new $dingtalkyida_1_0.CreateOrUpdateFormDataRequest({
      noExecuteExpression: false,
      formUuid: "FORM-GX866MC1NC1VOFF6WVQW33FD16E23L3CPMKVKA",
      searchCondition: "[{\"key\":\"currentNodeName\",\"value\":\"当前审批节点名称\",\"type\":\"TEXT\",\"operator\":\"like\",\"componentName\":\"TextField\"}]。详情参考 https://www.yuque.com/yida/support/agb8im#F4S8e",
      appType: "APP_XCE0EVXS6DYG3YDYC5RD",
...
namespace AlibabaCloud.SDK.Sample
{
    public class Sample 
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AlibabaCloud.SDK.Dingtalkyida_1_0.Models.CreateOrUpdateFormDataHeaders createOrUpdateFormDataHeaders = new AlibabaCloud.SDK.Dingtalkyida_1_0.Models.CreateOrUpdateFormDataHeaders();
            createOrUpdateFormDataHeaders.XAcsDingtalkAccessToken = "<your access token>";
            AlibabaCloud.SDK.Dingtalkyida_1_0.Models.CreateOrUpdateFormDataRequest createOrUpdateFormDataRequest = new AlibabaCloud.SDK.Dingtalkyida_1_0.Models.CreateOrUpdateFormDataRequest
            {
                NoExecuteExpression = false,
                FormUuid = "FORM-GX866MC1NC1VOFF6WVQW33FD16E23L3CPMKVKA",
                SearchCondition = "[{\"key\":\"currentNodeName\",\"value\":\"当前审批节点名称\",\"type\":\"TEXT\",\"operator\":\"like\",\"componentName\":\"TextField\"}]。详情参考 https://www.yuque.com/yida/support/agb8im#F4S8e",
