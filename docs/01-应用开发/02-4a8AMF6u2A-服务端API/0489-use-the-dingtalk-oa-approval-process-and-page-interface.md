...
group: "应用开发"
tab: "服务端API"
breadcrumb: "OA 审批 > 使用教程 > 官方OA 审批 > 官方OA审批：钉钉流程与页面对接"
doc_id: "Dv4RnLILxO"
updated_at: "2026-07-10 10:11:37"
---

> Source: https://open.dingtalk.com/document/development/use-the-dingtalk-oa-approval-process-and-page-interface
> Path: 应用开发 / 服务端API / OA 审批 > 使用教程 > 官方OA 审批 > 官方OA审批：钉钉流程与页面对接
> Updated: 2026-07-10 10:11:37
...
- 支持从业务系统发起流程，调用钉钉OA审批接口创建OA审批流程
  ...
  ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7949463871/p838390.png)
- 支持从钉钉OA审批列表打开官方OA审批详情页审批

  ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7949463871/p838406.png)
- 支持从钉钉待办打开官方OA审批详情页审批

  ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7949463871/p838407.png)

## 开发流程

### **流程图**

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7949463871/p835415.png)
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7949463871/p838408.png)
...
3. **发起官方OA审批实例：**用户可通过OA审批官方应用手动发起审批；或业务系统可根据模板编码`processCode`，调用新版服务端API-[发起审批实例](0497-create-an-approval-instance.md)接口发起审批实例，获取审批实例`instanceId`。
...
6. **预览下载审批附件：**若需要预览/下载附件，可调用服务端API-[授权下载审批钉盘文件](0504-download-the-approval-nail-file.md)接口、[授权预览审批附件](0503-official-authorized-preview-approval-attachment.md)接口，进行审批钉盘文件的授权操作，再调用服务端API-[下载审批附件](0505-download-an-approval-attachment.md)接口，获取文件的链接`downloadUri`实现下载。具体使用教程参考：[审批附件的操作流程](0487-new-version-of-attachment-approval-process.md)。
7. **同意/拒绝审批任务：**审批人可通过钉钉OA审批中心、钉钉待办中心手动操作同意/拒绝审批任务；或业务系统根据审批实例`instanceId`，调用新版服务端API-[获取单个审批实例详情](0498-obtains-the-details-of-a-single-approval-instance-pop.md)接口，获取审批实例详情，获取审批任务各个任务节点信息`taskId`。
8. 根据审批实例`instanceId`和相应的任务节点`taskId`信息，调用新版服务端API-[同意或拒绝审批任务](0506-approve-or-reject-the-approval-task.md)接口，实现审批任务的操作，所有审批节点同意后，则该审批单通过。
...
9. 撤销官方OA审批实例：查看审批后，若发现提交的审批单有误需撤销该审批实例，可根据审批实例`instanceId`，调用新版服务端API-[撤销审批实例](0499-revoke-an-approval-instance.md)，实现审批单的撤销操作。
...
10. 审批单状态发生变化后，OA审批支持将[审批任务开始，结束，转交](../04-LFcRvVD08N-事件订阅/0038-event-bpms-task-change.md)和[审批实例开始、结束、终止、删除](../04-LFcRvVD08N-事件订阅/0039-event-bpms-instance-change.md)等回调事件推送至业务系统侧，可以让企业应用能够更深度地与钉钉平台集成，实现信息共享和业务协同。具体使用教程参考：[事件订阅操作指南](0014-event-subscription-overview.md)。
