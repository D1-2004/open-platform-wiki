...
| id | String | 是 | 权限成员id：   - `type=ORG`时，id为企业id - `type=DEPT`时，id为部门id - `type=TAG`时，id为标签id - `type=CONVERSATION`时，id为会话id - `type=USER`时，id为员工id - `type=DYNAMIC_GROUP`时，id为动态用户组groupCode |
