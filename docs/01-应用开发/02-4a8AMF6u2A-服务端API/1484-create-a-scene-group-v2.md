...
> **[!IMPORTANT]**
>
> 当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[创建场景群](0746-create-a-scene-group.md)接口，已接入的开发者结合实际尽快完成迁移。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | beE4\*\*\*\*\*75d | 调用该API的应用凭证。   - 企业内部应用，通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 - 第三方企业应用，通过[获取第三方企业的access\_token](1445-obtain-isvapp-token.md)接口获取。 |
...
| icon | String | 否 | @lADOADma\*\*\*\*\*QKA | 群头像，格式为`mediaId`。需要调用[上传媒体文件](0646-upload-media-files.md)接口上传群头像，获取mediaId。 |
