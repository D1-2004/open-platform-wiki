...
doc_id: "jcPeRP46Rc"
updated_at: "2026-05-19 11:19:45"
---
...
2. 实现应用免登。
   ...
   1. 推送suiteTicket。钉钉支持以下两种推送方式：
      ...
      - [配置RDS](https://open.dingtalk.com/document/development/configure-an-rds-push-table)
      - [配置 SyncHTTP 推送（不推荐）](../04-LFcRvVD08N-事件订阅/0003-configure-stream-push.md#421584309ds03)
...
- **数据变更推送**
  ...
  - RDS推送：
    ...
    在保障数据安全的前提下，极大的简化了推送协议，减少了数据传输次数，提高了数据传输速度，提升了推送的稳定性。详情请参考[RDS推送概述](https://open.dingtalk.com/document/development/configure-an-rds-push-table)。
  - SyncHTTP回调推送：
...
- **消息通知**
  ...
  - **任务类通知**：是指需要发送一条任务提醒给员工，比如审批任务等，这类情况下可参考[创建钉钉待办任务](0791-add-dingtalk-to-do-task.md)。

    消息类型和样例可参考[消息通知类型](0773-message-types-and-data-format.md)文档。
- 新教育相关接口：
  ...
  - **家校通讯录接口：**
    ...
    文档详见：[调用说明（家校通讯录2.0）](https://open.dingtalk.com/document/development/call-instructions-home-school-address-book-2-0)
...
5. 定制服务商集钉钉API。
   ...
   - **消息通知**
     ...
     - **群消息**：是指可以调用接口以系统名义向群里推送群聊消息。
     - **普通消息**：是指员工个人在使用应用时，可以通过界面操作的方式往群或其他人的会话里推送消息，例如发送日志的场景。
     - **任务类通知**：是指需要发送一条任务提醒给员工，比如审批任务等，这类情况请参考[创建钉钉待办任务](0791-add-dingtalk-to-do-task.md)。
   ...
   - **新教育API**
     ...
     - **家校通讯录接口：**
       ...
       我们提供了集成教育相关接口的的demo，demo地址：[调用说明（家校通讯录2.0）](https://open.dingtalk.com/document/development/call-instructions-home-school-address-book-2-0)
