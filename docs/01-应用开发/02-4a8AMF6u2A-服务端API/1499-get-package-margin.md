---
title: "获取套餐余量"
source_url: "https://open.dingtalk.com/document/development/get-package-margin"
namespace: "development"
slug: "get-package-margin"
group: "应用开发"
tab: "服务端API"
...
当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[查询套餐余量](1075-query-package-balance.md)接口，已接入的开发者结合实际尽快完成迁移。
...
public class Sample {
    ...
    public static void main(String[] args_) throws Exception {
        ...
        ContractMarginHeaders contractMarginHeaders = new ContractMarginHeaders();
        contractMarginHeaders.xAcsDingtalkAccessToken = "<your access token>";
        try {
            client.contractMarginWithOptions(contractMarginHeaders, new RuntimeOptions());
...
class Sample:
    ...
    def main(
        ...
        contract_margin_headers = dingtalkesign__1__0_models.ContractMarginHeaders()
        contract_margin_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            client.contract_margin_with_options(contract_margin_headers, util_models.RuntimeOptions())
    ...
    async def main_async(
        ...
        contract_margin_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            await client.contract_margin_with_options_async(contract_margin_headers, util_models.RuntimeOptions())
        except Exception as err:
            if not UtilClient.empty(err.code) and not UtilClient.empty(err.message):
                # err 中含有 code 和 message 属性，可帮助开发定位问题
                pass
...
class Sample {
    ...
    public static function main($args){
        ...
        $contractMarginHeaders = new ContractMarginHeaders([]);
        $contractMarginHeaders->xAcsDingtalkAccessToken = "<your access token>";
        try {
            $client->contractMarginWithOptions($contractMarginHeaders, new RuntimeOptions([]));
        }
...
func _main (args []*string) (_err error) {
  ...
  contractMarginHeaders := &dingtalkesign_1_0.ContractMarginHeaders{}
  contractMarginHeaders.XAcsDingtalkAccessToken = tea.String("<your access token>")
  tryErr := func()(_e error) {
    ...
    _, _err = client.ContractMarginWithOptions(contractMarginHeaders, &util.RuntimeOptions{})
    if _err != nil {
      return _err
    }
...
export default class Client {
  ...
  static async main(args: string[]): Promise<void> {
    ...
    let contractMarginHeaders = new $dingtalkesign_1_0.ContractMarginHeaders({ });
    contractMarginHeaders.xAcsDingtalkAccessToken = "<your access token>";
    try {
      await client.contractMarginWithOptions(contractMarginHeaders, new $Util.RuntimeOptions({ }));
    } catch (err) {
...
namespace AlibabaCloud.SDK.Sample
{
    public class Sample 
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AlibabaCloud.SDK.Dingtalkesign_1_0.Models.ContractMarginHeaders contractMarginHeaders = new AlibabaCloud.SDK.Dingtalkesign_1_0.Models.ContractMarginHeaders();
            contractMarginHeaders.XAcsDingtalkAccessToken = "<your access token>";
            try
            {
                client.ContractMarginWithOptions(contractMarginHeaders, new AlibabaCloud.TeaUtil.Models.RuntimeOptions());
            }
            catch (TeaException err)
            {
...
int main(int argc, char *args[]) {
  ...
  shared_ptr<Alibabacloud_Dingtalkesign_1_0::ContractMarginHeaders> contractMarginHeaders = make_shared<Alibabacloud_Dingtalkesign_1_0::ContractMarginHeaders>();
  contractMarginHeaders->xAcsDingtalkAccessToken = make_shared<string>("<your access token>");
  try {
    client->contractMarginWithOptions(contractMarginHeaders, make_shared<Darabonba_Util::RuntimeOptions>(Darabonba_Util::RuntimeOptions()));
  }
