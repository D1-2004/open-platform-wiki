...
group: "应用开发"
tab: "服务端API"
breadcrumb: "OA 审批 > 使用教程 > 官方OA 审批 > 发起及撤销审批流"
doc_id: "s7yjG9gPJc"
updated_at: "2026-07-10 10:11:29"
---

> Source: https://open.dingtalk.com/document/development/new-version-of-initiate-and-cancel-approval-flow
> Path: 应用开发 / 服务端API / OA 审批 > 使用教程 > 官方OA 审批 > 发起及撤销审批流
> Updated: 2026-07-10 10:11:29
...
3. 根据审批实例`instanceId`，调用新版服务端API-[获取单个审批实例详情](0498-obtains-the-details-of-a-single-approval-instance-pop.md)接口，获取审批实例详情信息。
4. 根据审批实例详情信息字段参数，调用新版服务端API-[转交OA审批任务](0507-transfer-the-oa-approval-task.md)接口，实现审批实例的转交操作。
5. 查看审批后，可根据审批实例`instanceId`，调用新版服务端API-[撤销审批实例](0499-revoke-an-approval-instance.md)，实现审批单的撤销操作。
...
2. 根据模板编码`processCode`，调用新版服务端API-[发起审批实例](0497-create-an-approval-instance.md)接口发起审批实例，获取审批实例`instanceId`。
   ...
   ```
3. 根据审批实例`instanceId`，调用新版服务端API-[获取单个审批实例详情](0498-obtains-the-details-of-a-single-approval-instance-pop.md)接口，获取审批实例详情。
