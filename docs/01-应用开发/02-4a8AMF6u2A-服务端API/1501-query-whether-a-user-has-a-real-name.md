...
slug: "query-whether-a-user-has-a-real-name"
group: "应用开发"
tab: "服务端API"
...
当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[查询个人是否实名认证](1079-query-personal-information.md)接口，已接入的开发者结合实际尽快完成迁移。
...
public class Sample {
    ...
    public static void main(String[] args_) throws Exception {
        ...
        getUserInfoHeaders.xAcsDingtalkAccessToken = "<your access token>";
        try {
            client.getUserInfoWithOptions("user123", getUserInfoHeaders, new RuntimeOptions());
...
class Sample:
    ...
    def main(
        ...
        get_user_info_headers = dingtalkesign__1__0_models.GetUserInfoHeaders()
        get_user_info_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            client.get_user_info_with_options('user123', get_user_info_headers, util_models.RuntimeOptions())
    ...
    async def main_async(
        ...
        get_user_info_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            await client.get_user_info_with_options_async('user123', get_user_info_headers, util_models.RuntimeOptions())
        except Exception as err:
            if not UtilClient.empty(err.code) and not UtilClient.empty(err.message):
                # err 中含有 code 和 message 属性，可帮助开发定位问题
                pass
...
class Sample {
    ...
    public static function main($args){
        ...
        $getUserInfoHeaders = new GetUserInfoHeaders([]);
        $getUserInfoHeaders->xAcsDingtalkAccessToken = "<your access token>";
        try {
            $client->getUserInfoWithOptions("user123", $getUserInfoHeaders, new RuntimeOptions([]));
        }
...
func _main (args []*string) (_err error) {
  ...
  getUserInfoHeaders := &dingtalkesign_1_0.GetUserInfoHeaders{}
  getUserInfoHeaders.XAcsDingtalkAccessToken = tea.String("<your access token>")
  tryErr := func()(_e error) {
    ...
    _, _err = client.GetUserInfoWithOptions(tea.String("user123"), getUserInfoHeaders, &util.RuntimeOptions{})
    if _err != nil {
      return _err
    }
...
export default class Client {
  ...
  static async main(args: string[]): Promise<void> {
    ...
    let getUserInfoHeaders = new $dingtalkesign_1_0.GetUserInfoHeaders({ });
    getUserInfoHeaders.xAcsDingtalkAccessToken = "<your access token>";
    try {
      await client.getUserInfoWithOptions("user123", getUserInfoHeaders, new $Util.RuntimeOptions({ }));
    } catch (err) {
...
namespace AlibabaCloud.SDK.Sample
{
    public class Sample 
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AlibabaCloud.SDK.Dingtalkesign_1_0.Models.GetUserInfoHeaders getUserInfoHeaders = new AlibabaCloud.SDK.Dingtalkesign_1_0.Models.GetUserInfoHeaders();
            getUserInfoHeaders.XAcsDingtalkAccessToken = "<your access token>";
            try
            {
                client.GetUserInfoWithOptions("user123", getUserInfoHeaders, new AlibabaCloud.TeaUtil.Models.RuntimeOptions());
            }
            catch (TeaException err)
            {
...
int main(int argc, char *args[]) {
  ...
  shared_ptr<Alibabacloud_Dingtalkesign_1_0::GetUserInfoHeaders> getUserInfoHeaders = make_shared<Alibabacloud_Dingtalkesign_1_0::GetUserInfoHeaders>();
  getUserInfoHeaders->xAcsDingtalkAccessToken = make_shared<string>("<your access token>");
  try {
    client->getUserInfoWithOptions(make_shared<string>("user123"), getUserInfoHeaders, make_shared<Darabonba_Util::RuntimeOptions>(Darabonba_Util::RuntimeOptions()));
  }
