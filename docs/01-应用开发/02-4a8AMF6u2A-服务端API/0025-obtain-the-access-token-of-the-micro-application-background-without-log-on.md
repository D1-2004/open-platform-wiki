...
slug: "obtain-the-access-token-of-the-micro-application-background-without-log-on"
group: "应用开发"
tab: "服务端API"
...
> Path: 应用开发 / 服务端API / 认证与授权 > 身份验证（免登） > 获取微应用后台免登的accessToken
> Updated: 2026-04-29 22:27:34

# 获取微应用后台免登的accessToken

调用本接口获取微应用后台免登的accessToken。获取的accessToken只在应用管理后台免登场景中使用。

## **请求**

| **基本信息** | |
| --- | --- |
| HTTP URL | https://api.dingtalk.com/v1.0/oauth2/ssoAccessToken |
| HTTP Method | POST |
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| corpid | String | 是 | corpxxxx | 企业的corpId。 |
| ssoSecret | String | 是 | shYgsk7xxx | sso密钥，可以在[开发者后台](https://open-dev.dingtalk.com/fe/old#/corpAuthInfo)**基本信息**—**开发信息**（**旧版**）页面查看。 |
...
```
curl -X POST 'https://api.dingtalk.com/v1.0/oauth2/ssoAccessToken' \
...
class Sample:
    ...
    def main(
        ...
        get_sso_access_token_request = dingtalkoauth_2__1__0_models.GetSsoAccessTokenRequest(
            corpid='corpxxxx',
            sso_secret='shYgsk7xxx'
        )
        try:
            client.get_sso_access_token(get_sso_access_token_request)
    ...
    async def main_async(
        ...
        try:
            await client.get_sso_access_token_async(get_sso_access_token_request)
        except Exception as err:
            if not UtilClient.empty(err.code) and not UtilClient.empty(err.message):
                # err 中含有 code 和 message 属性，可帮助开发定位问题
                pass
...
class Sample {
    ...
    public static function main($args){
        ...
        $getSsoAccessTokenRequest = new GetSsoAccessTokenRequest([
            "corpid" => "corpxxxx",
            "ssoSecret" => "shYgsk7xxx"
        ]);
        try {
...
func _main (args []*string) (_err error) {
  ...
  getSsoAccessTokenRequest := &dingtalkoauth2_1_0.GetSsoAccessTokenRequest{
    Corpid: tea.String("corpxxxx"),
    SsoSecret: tea.String("shYgsk7xxx"),
  }
  tryErr := func()(_e error) {
    ...
    _, _err = client.GetSsoAccessToken(getSsoAccessTokenRequest)
    if _err != nil {
      return _err
    }
...
export default class Client {
  ...
  static async main(args: string[]): Promise<void> {
    ...
    let getSsoAccessTokenRequest = new $dingtalkoauth2_1_0.GetSsoAccessTokenRequest({
      corpid: "corpxxxx",
      ssoSecret: "shYgsk7xxx",
    });
    try {
      await client.getSsoAccessToken(getSsoAccessTokenRequest);
    } catch (err) {
...
| 名称 | 类型 | 示例值 | 描述 |
| --- | --- | --- | --- |
| accessToken | String | 1234 | 微应用后台免登的accessToken。 |
| expireIn | Long | 3600 | 超时时间。 |
