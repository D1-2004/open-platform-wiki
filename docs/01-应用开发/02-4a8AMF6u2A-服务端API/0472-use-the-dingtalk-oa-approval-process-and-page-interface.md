...
source_url: "https://open.dingtalk.com/document/development/use-the-dingtalk-oa-approval-process-and-page-interface"
namespace: "development"
slug: "use-the-dingtalk-oa-approval-process-and-page-interface"
group: "应用开发"
tab: "服务端API"
breadcrumb: "OA 审批 > 使用教程 > 官方OA审批集成模式：使用钉钉OA审批流程和页面对接"
doc_id: "Dv4RnLILxO"
updated_at: "2025-09-10 19:31:45"
---

> Source: https://open.dingtalk.com/document/development/use-the-dingtalk-oa-approval-process-and-page-interface
> Path: 应用开发 / 服务端API / OA 审批 > 使用教程 > 官方OA审批集成模式：使用钉钉OA审批流程和页面对接
> Updated: 2025-09-10 19:31:45

# 官方OA审批集成模式：使用钉钉OA审批流程和页面对接
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3716224271/p835400.png)

## **实现效果**

### **支持从业务系统发起流程，调用钉钉OA审批接口创建OA审批流程**

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3716224271/p838390.png)

### 支持从钉钉OA审批列表打开官方OA审批详情页审批

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3716224271/p838406.png)

### **支持从钉钉待办打开官方OA审批详情页审批**

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3716224271/p838407.png)

## 开发流程

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3716224271/p835415.png)
...
前提条件：完成[创建应用](../01-XOnnmGCTbn-开发指南/0007-create-application.md)的流程。
...
1. **创建官方OA审批模板：**管理员可在OA管理后台手动操作创建；或业务系统调用新版服务端API-[创建或更新审批表单模板](0487-create-an-approval-form-template.md)接口，获取模板的唯一编码`processCode`。若没有保存接口返回的模板编码`processCode`，可登录钉钉管理后台查看获取。

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3716224271/p838408.png)
...
3. **发起官方OA审批实例：**用户可通过OA审批官方应用手动发起审批；或业务系统可根据模板编码`processCode`，调用新版服务端API-[发起审批实例](0493-create-an-approval-instance.md)接口发起审批实例，获取审批实例`instanceId`。
...
5. **添加审批评论附件：**若需要添加审批评论附件，需先将文件上传至审批钉盘空间，再调用新版服务端API-[添加审批评论](https://open.dingtalk.com/document/development/add-an-approval-comment-pop)接口。具体使用教程参考：[评论及撤销审批流](0481-comment-and-revoke-approval-flow.md)。

   1. 需先调用新版服务端API-[获取审批钉盘空间信息](0498-obtains-the-information-about-approval-nail-disk.md)接口，获取钉盘空间的上传权限，并获取审批钉盘空间spaceId。
   2. 调用客户端JSAPI-[uploadAttachmentToDingTalk](../03-Ogu5SlPY4t-客户端JSAPI/0333-jsapi-upload-attachment-to-ding-talk.md)接口，获取文件基本信息，本流程示例使用[JSAPI Explorer](https://open.dingtalk.com/tools/explorer/jsapi?id=10318)实现。
   3. 获取审批钉盘空间spaceId后，可根据审批实例`instanceId`，调用新版服务端API-[添加审批评论](https://open.dingtalk.com/document/development/add-an-approval-comment-pop)接口，实现审批单的添加评论操作。
...
6. **预览下载审批附件：**若需要预览/下载附件，可调用服务端API-[授权下载审批钉盘文件](0500-download-the-approval-nail-file.md)接口、[授权预览审批附件](https://open.dingtalk.com/document/development/preview-authorization-attachment-pop)接口，进行审批钉盘文件的授权操作，再调用服务端API-[下载审批附件](0501-download-an-approval-attachment.md)接口，获取文件的链接`downloadUri`实现下载。具体使用教程参考：[审批附件的操作流程](0483-new-version-of-attachment-approval-process.md)。
7. **同意/拒绝审批任务：**审批人可通过钉钉OA审批中心、钉钉待办中心手动操作同意/拒绝审批任务；或业务系统根据审批实例`instanceId`，调用新版服务端API-[获取单个审批实例详情](0494-obtains-the-details-of-a-single-approval-instance-pop.md)接口，获取审批实例详情，获取审批任务各个任务节点信息`taskId`。
8. 根据审批实例`instanceId`和相应的任务节点`taskId`信息，调用新版服务端API-[同意或拒绝审批任务](0502-approve-or-reject-the-approval-task.md)接口，实现审批任务的操作，所有审批节点同意后，则该审批单通过。
...
9. 撤销官方OA审批实例：查看审批后，若发现提交的审批单有误需撤销该审批实例，可根据审批实例`instanceId`，调用新版服务端API-[撤销审批实例](0495-revoke-an-approval-instance.md)，实现审批单的撤销操作。
...
10. 审批单状态发生变化后，OA审批支持将[审批任务状态变化](../04-LFcRvVD08N-事件订阅/0032-event-bpms-task-change.md)和[审批实例状态变化](../04-LFcRvVD08N-事件订阅/0033-event-bpms-instance-change.md)等回调事件推送至业务系统侧，可以让企业应用能够更深度地与钉钉平台集成，实现信息共享和业务协同。具体使用教程参考：[事件订阅操作指南](0014-event-subscription-overview.md)。

## 客户案例

<https://page.dingtalk.com/wow/tianyuan/act/toufang?wh_showError=true&caseId=NTMzNg==>

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3716224271/p835426.png)
