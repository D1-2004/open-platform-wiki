...
开发者可以通过钉钉 AI 助理主动给用户发送自定义智能消息。主动发送的步骤可以分为：[预备](#)、[更新](1593-the-ai-assistant-updates-active-message-sending-mode.md)和[结束](1594-api-finish.md)三步。本文档介绍的接口是预备接口，该接口可以给用户发送一个状态为“准备中”的消息框，让用户有一个良好的交互体验，同时开发者能收到会话凭证，用于后续的消息更新和结束。
