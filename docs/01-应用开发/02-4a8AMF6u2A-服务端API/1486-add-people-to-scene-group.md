...
doc_id: "CNYIgjCmoD"
updated_at: "2026-05-13 15:50:34"
---
...
> **[!IMPORTANT]**
>
> 当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[添加群成员](0750-api-addscenegroupmember.md)接口，已接入的开发者结合实际尽快完成迁移。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6d1b\*\*\*\*\* | 调用该API的应用凭证。   - 企业内部应用，通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 - 第三方企业应用，通过[获取第三方企业的access\_token](1445-obtain-isvapp-token.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| open\_conversation\_id | String | 是 | cid9FTRQSLo+sK\*\*\*\*\* | 群ID，调用[创建群](1484-create-a-scene-group-v2.md)接口获取`open_conversation_id`参数值。 |
