...
slug: "knowledge-base-download-file"
group: "应用开发"
tab: "服务端API"
breadcrumb: "文档/文件 > 知识库 > 知识库目录树管理 > 知识库下载文件"
doc_id: "hBAt8HEEsv"
updated_at: "2026-06-03 09:48:34"
---

> Source: https://open.dingtalk.com/document/development/knowledge-base-download-file
> Path: 应用开发 / 服务端API / 文档/文件 > 知识库 > 知识库目录树管理 > 知识库下载文件
> Updated: 2026-06-03 09:48:34
...
> **[!NOTE]**
>
> 如果文件过大，需要分段获取资源，可在上述示例中添加headers字段：`headers.put("Range","bytes=0-499");`，具体请参考[通过指定Range下载部分内容](0677-use-range-download-a-part-of-content.md)。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| spaceId | String | 是 | 空间Id，可通过[获取节点列表](0568-get-node-list.md)接口获取知识库节点的 `dentryUuid`，再调用[根据 dentryUuid 获取 spaceId](0671-api-getdentryidbyuuid.md)接口获取 `spaceId`。 |
| dentryId | String | 是 | 文件Id，可通过[获取节点列表](0568-get-node-list.md)接口获取知识库节点的 `dentryUuid`，再调用[根据 dentryUuid 获取 spaceId](0671-api-getdentryidbyuuid.md)接口获取 `dentryId`。 |

### 查询参数

| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| unionId | String | 是 | 操作者unionId，可调用[查询用户详情](0056-query-user-details.md)接口获取。 |
...
| version | Long | 否 | 文件的版本号，可调用[获取文件版本列表](0661-obtains-a-list-of-file-versions.md)接口获取version。 |
| preferIntranet | Boolean | 否 | 是否优先使用内网传输，使用该参数前提是配置了专属存储内网传输。   - **true**（默认值）：是 - **false**：否 |
...
| headerSignatureInfo | Object | Header加签信息。  **[!NOTE]**  当protocol字段值为HEADER\_SIGNATURE时，此字段生效。 |
...
| internalResourceUrls | Array of String | 内网URL。  **[!NOTE]**  该字段目前无使用场景，请忽略。 |
