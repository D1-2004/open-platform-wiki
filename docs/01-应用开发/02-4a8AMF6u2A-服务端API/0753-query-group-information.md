...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| openConversationId | String | 是 | 群ID：   - 基于群模板创建的群：调用[创建场景群](0744-create-a-scene-group.md)接口获取`open_conversation_id`参数值。 - 安装群聊酷应用的群：通过[群内安装酷应用事件](../01-XOnnmGCTbn-开发指南/0058-group-chat-coolapp-event.md)获取回调参数`OpenConversationId`参数值。 |
