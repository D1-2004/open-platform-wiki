---
title: "删除H5微应用"
source_url: "https://open.dingtalk.com/document/development/delete-an-h5-microapplication"
namespace: "development"
slug: "delete-an-h5-microapplication"
group: "应用开发"
tab: "服务端API"
breadcrumb: "历史文档（不推荐） > 应用管理 > 删除H5微应用"
doc_id: "04ZuBzGhPg"
updated_at: "2025-09-08 19:05:21"
---

> Source: https://open.dingtalk.com/document/development/delete-an-h5-microapplication
> Path: 应用开发 / 服务端API / 历史文档（不推荐） > 应用管理 > 删除H5微应用
> Updated: 2025-09-08 19:05:21

# 删除H5微应用

调用本接口删除H5微应用。

> **[!IMPORTANT]**
>
> 为提升接口使用体验，针对**应用管理**相关接口规范进行升级，从[旧版升级到新版](https://open.dingtalk.com/document/orgapp/differences-between-server-apis-and-new-server-apis)。本文旧版规范接口文档已于**2022年6月24日**迁移至**历史文档（不推荐）**目录，且**本接口仅保持现有功能，不再新增支持其他能力。**
>
> - 如果未使用本接口，推荐使用新版规范[删除企业内部应用H5微应用](https://open.dingtalk.com/document/orgapp/delete-an-internal-h5-application)接口。
> - 如果已使用本接口，建议您根据自身实际情况评估是否切换至推荐接口。

例如，调用接口删除“测试删除应用”，调用接口前应用列表如下图所示。![应用管理-删除应用3](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7268917361/p354758.png)

调用接口后，删除“测试删除应用”应用，在应用列表不再展示，如下图所示。

![应用管理-删除应用2](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7268917361/p354757.png)

## 权限

服务端API是以应用维度授权的，在调用接口前，确保已经为应用添加了接口权限。详情请参考[权限申请](https://open.dingtalk.com/document/app/application-apply-for-permissions#topic-2026047)。

| 应用类型 | 是否支持调用 | 权限申请方式 | API Explorer调试 |
| --- | --- | --- | --- |
| 企业内部应用 | 是 | **[!IMPORTANT]**  暂不支持新增申请。 | — |
| 第三方企业应用 | 否 | — | — |
| 第三方个人应用 | 否 | — | — |

## 基本信息

**请求方式**：POST

**请求地址**：`https://oapi.dingtalk.com/microapp/delete`

## Query参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6d1bxxxx | 调用服务端API的应用凭证。   - 企业内部应用可通过[获取企业内部应用的access\_token](https://open.dingtalk.com/document/orgapp/obtain-orgapp-token)接口获取 |

## Body参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| agentId | Number | 是 | 1144651083 | H5微应用的agentid。企业只能删除自建H5微应用。 |

## 返回参数

| 名称 | 类型 | 示例值 | 描述 |
| --- | --- | --- | --- |
| errmsg | String | ok | 返回码描述。 |
| errcode | Number | 0 | 返回码。 |

## 示例

**请求示例（HTTP）**

```
POST https://oapi.dingtalk.com/microapp/delete?access_token=ACCESS_TOKEN
```

请求正文

```
{
        "agentId":1144651083
}
```

**请求示例（JAVA SDK）**

```
DingTalkClient client = new DefaultDingTalkClient("https://oapi.dingtalk.com/microapp/delete");
OapiMicroappDeleteRequest req = new OapiMicroappDeleteRequest();
req.setAgentId(1144651083L);
OapiMicroappDeleteResponse rsp = client.execute(req, access_token);
System.out.println(rsp.getBody());
```

**返回示例**

```
{
    "errcode": 0,
    "errmsg": "ok",
    "agentId": 1144651083
}
```
