...
slug: "check-whether-enterprise-registered-realname-e-sign-treasure"
group: "应用开发"
tab: "服务端API"
breadcrumb: "历史文档（不推荐） > e签宝 1.0 > 用户 > 查询企业是否实名"
doc_id: "W8ankZjVD2"
updated_at: "2025-12-05 19:17:24"
---
...
当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[查询企业是否实名认证](1077-query-enterprise-information.md)接口，已接入的开发者结合实际尽快完成迁移。
...
public class Sample {
    ...
    public static void main(String[] args_) throws Exception {
        ...
        CorpInfoHeaders corpInfoHeaders = new CorpInfoHeaders();
        corpInfoHeaders.xAcsDingtalkAccessToken = "<your access token>";
        try {
            client.corpInfoWithOptions(corpInfoHeaders, new RuntimeOptions());
...
class Sample:
    ...
    def main(
        ...
        corp_info_headers = dingtalkesign__1__0_models.CorpInfoHeaders()
        corp_info_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            client.corp_info_with_options(corp_info_headers, util_models.RuntimeOptions())
    ...
    async def main_async(
        ...
        corp_info_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            await client.corp_info_with_options_async(corp_info_headers, util_models.RuntimeOptions())
        except Exception as err:
            if not UtilClient.empty(err.code) and not UtilClient.empty(err.message):
                # err 中含有 code 和 message 属性，可帮助开发定位问题
                pass
...
class Sample {
    ...
    public static function main($args){
        ...
        $corpInfoHeaders = new CorpInfoHeaders([]);
        $corpInfoHeaders->xAcsDingtalkAccessToken = "<your access token>";
        try {
            $client->corpInfoWithOptions($corpInfoHeaders, new RuntimeOptions([]));
        }
...
func _main (args []*string) (_err error) {
  ...
  corpInfoHeaders := &dingtalkesign_1_0.CorpInfoHeaders{}
  corpInfoHeaders.XAcsDingtalkAccessToken = tea.String("<your access token>")
  tryErr := func()(_e error) {
    ...
    _, _err = client.CorpInfoWithOptions(corpInfoHeaders, &util.RuntimeOptions{})
    if _err != nil {
      return _err
    }
...
export default class Client {
  ...
  static async main(args: string[]): Promise<void> {
    ...
    let corpInfoHeaders = new $dingtalkesign_1_0.CorpInfoHeaders({ });
    corpInfoHeaders.xAcsDingtalkAccessToken = "<your access token>";
    try {
      await client.corpInfoWithOptions(corpInfoHeaders, new $Util.RuntimeOptions({ }));
    } catch (err) {
...
namespace AlibabaCloud.SDK.Sample
{
    public class Sample 
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AlibabaCloud.SDK.Dingtalkesign_1_0.Models.CorpInfoHeaders corpInfoHeaders = new AlibabaCloud.SDK.Dingtalkesign_1_0.Models.CorpInfoHeaders();
            corpInfoHeaders.XAcsDingtalkAccessToken = "<your access token>";
            try
            {
                client.CorpInfoWithOptions(corpInfoHeaders, new AlibabaCloud.TeaUtil.Models.RuntimeOptions());
            }
            catch (TeaException err)
            {
...
int main(int argc, char *args[]) {
  ...
  shared_ptr<Alibabacloud_Dingtalkesign_1_0::CorpInfoHeaders> corpInfoHeaders = make_shared<Alibabacloud_Dingtalkesign_1_0::CorpInfoHeaders>();
  corpInfoHeaders->xAcsDingtalkAccessToken = make_shared<string>("<your access token>");
  try {
    client->corpInfoWithOptions(corpInfoHeaders, make_shared<Darabonba_Util::RuntimeOptions>(Darabonba_Util::RuntimeOptions()));
  }
...
| orgRealName | String | 企业名称。      如果已经实名，会返回实名企业名称。 |
| code | Integer | 返回码。 |
