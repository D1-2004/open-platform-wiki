...
doc_id: "SLWFoAuUux"
updated_at: "2025-09-24 20:35:13"
---
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| appUserId | String | 是 | 钉外账号在业务系统内的标识，通过调用[创建钉钉客联钉外账号](1830-create-bc-account-association.md)接口获取，长度限制为1~64个字符。例如：1107\*\*\*\*2120。 |
...
| openConversationId | String | 否 | 群会话openConversationId，可调用[创建钉钉客联普通互通群](1831-create-common-group-new-version.md) / [创建钉钉客联两人互通群](1832-creating-two-groups-of-people.md)接口获取，长度限制为1～32个字符，例如：14da\*\*\*\*2760。 |
| channelCode | String | 是 | 渠道code，获取方式可查看[渠道配置](1825-interconnections-channel.md)文档，示例如：M0U+\*\*\*\*8Ep=。  **[!NOTE]**     - 如果当前code是移动端的渠道code，则返回值结果为移动端H5页面地址；若果是PC端的渠道code，则返回PC端H5页面。 - 不同渠道的code只会影响渲染效果，不会影响群列表的返回结果。 |
