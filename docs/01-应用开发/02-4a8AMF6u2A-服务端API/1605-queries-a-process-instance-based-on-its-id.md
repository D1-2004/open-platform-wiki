---
title: "根据流程实例ID获取流程实例"
source_url: "https://open.dingtalk.com/document/development/queries-a-process-instance-based-on-its-id"
namespace: "development"
slug: "queries-a-process-instance-based-on-its-id"
group: "应用开发"
tab: "服务端API"
breadcrumb: "历史文档（不推荐） > 宜搭 > 流程 > 根据流程实例ID获取流程实例"
doc_id: "zDziVBJrRQ"
updated_at: "2025-09-08 19:03:34"
---

> Source: https://open.dingtalk.com/document/development/queries-a-process-instance-based-on-its-id
> Path: 应用开发 / 服务端API / 历史文档（不推荐） > 宜搭 > 流程 > 根据流程实例ID获取流程实例
> Updated: 2025-09-08 19:03:34

# 根据流程实例ID获取流程实例

调用本接口根据指定流程实例ID获取流程实例详情。

> **[!IMPORTANT]**
>
> 为提升接口使用体验，针对宜搭部分接口进行升级，本文档预计于 2024 年 11 月 1 日迁移至历史文档（不推荐）目录，接口不再支持新应用接入, 已接入的应用可继续调用，且本接口仅保持现有功能，不再新增支持其他能力。
>
> - 如果未使用本接口，推荐使用[根据流程实例ID获取流程实例](https://open.dingtalk.com/document/orgapp/api-getinstancebyid-v2)接口。
> - 如果已使用本接口，建议您根据自身实际情况评估是否切换至推荐接口。

![](https://img.alicdn.com/imgextra/i2/O1CN01iD7VIq1GPFuer8bui_!!6000000000614-2-tps-1012-302.png)
...
```
GET /v1.0/yida/processes/instancesInfos/{id}?appType=String&systemToken=String&userId=String&language=String HTTP/1.1
...
| actionExecutor | Array | 流程实例当前任务执行人列表。 |
| name | Object | 名称。 |
...
| email | String | 邮箱。 |
| approvedResult | String | 审批结果，取值：   - agree：同意 - disagree：拒绝 |
...
| email | String | 邮箱。 |
| title | String | 流程标题。 |
| instanceStatus | String | 实例状态，取值：   - RUNNING：运行中 - TERMINATED：已终止 - COMPLETED：已完成 - ERROR：异常 |
| version | Long | 版本。 |
...
```
GET /v1.0/yida/processes/instancesInfos/f30233fb-72e1-xxx?appType=APP_PBKT0xxx&systemToken=hexxxx&userId=manager123&language=zh_CN HTTP/1.1
Host:api.dingtalk.com
...
public class Sample {
    ...
    public static void main(String[] args_) throws Exception {
        ...
        getInstanceByIdHeaders.xAcsDingtalkAccessToken = "<your access token>";
        GetInstanceByIdRequest getInstanceByIdRequest = new GetInstanceByIdRequest()
                ...
                .setLanguage("zh_CN");
        try {
            client.getInstanceByIdWithOptions("f30233fb-72e1-xxx", getInstanceByIdRequest, getInstanceByIdHeaders, new RuntimeOptions());
...
class Sample:
    ...
    def main(
        ...
        get_instance_by_id_headers = dingtalkyida__1__0_models.GetInstanceByIdHeaders()
        get_instance_by_id_headers.x_acs_dingtalk_access_token = '<your access token>'
        get_instance_by_id_request = dingtalkyida__1__0_models.GetInstanceByIdRequest(
        ...
        try:
            client.get_instance_by_id_with_options('f30233fb-72e1-xxx', get_instance_by_id_request, get_instance_by_id_headers, util_models.RuntimeOptions())
    ...
    async def main_async(
        ...
        try:
            await client.get_instance_by_id_with_options_async('f30233fb-72e1-xxx', get_instance_by_id_request, get_instance_by_id_headers, util_models.RuntimeOptions())
        except Exception as err:
            if not UtilClient.empty(err.code) and not UtilClient.empty(err.message):
                # err 中含有 code 和 message 属性，可帮助开发定位问题
                pass
...
func _main (args []*string) (_err error) {
  ...
  getInstanceByIdHeaders := &dingtalkyida_1_0.GetInstanceByIdHeaders{}
  getInstanceByIdHeaders.XAcsDingtalkAccessToken = tea.String("<your access token>")
  getInstanceByIdRequest := &dingtalkyida_1_0.GetInstanceByIdRequest{
  ...
  tryErr := func()(_e error) {
    ...
    _, _err = client.GetInstanceByIdWithOptions(tea.String("f30233fb-72e1-xxx"), getInstanceByIdRequest, getInstanceByIdHeaders, &util.RuntimeOptions{})
    if _err != nil {
      return _err
    }
...
export default class Client {
  ...
  static async main(args: string[]): Promise<void> {
    ...
    let getInstanceByIdHeaders = new $dingtalkyida_1_0.GetInstanceByIdHeaders({ });
    getInstanceByIdHeaders.xAcsDingtalkAccessToken = "<your access token>";
    let getInstanceByIdRequest = new $dingtalkyida_1_0.GetInstanceByIdRequest({
      appType: "APP_PBKT0xxx",
      systemToken: "hexxxx",
      userId: "manager123",
      language: "zh_CN",
    ...
    try {
      await client.getInstanceByIdWithOptions("f30233fb-72e1-xxx", getInstanceByIdRequest, getInstanceByIdHeaders, new $Util.RuntimeOptions({ }));
    } catch (err) {
...
namespace AlibabaCloud.SDK.Sample
{
    public class Sample 
    {
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                client.GetInstanceByIdWithOptions("f30233fb-72e1-xxx", getInstanceByIdRequest, getInstanceByIdHeaders, new AlibabaCloud.TeaUtil.Models.RuntimeOptions());
            }
            catch (TeaException err)
            {
...
int main(int argc, char *args[]) {
  ...
  shared_ptr<Alibabacloud_Dingtalkyida_1_0::GetInstanceByIdHeaders> getInstanceByIdHeaders = make_shared<Alibabacloud_Dingtalkyida_1_0::GetInstanceByIdHeaders>();
  getInstanceByIdHeaders->xAcsDingtalkAccessToken = make_shared<string>("<your access token>");
  shared_ptr<Alibabacloud_Dingtalkyida_1_0::GetInstanceByIdRequest> getInstanceByIdRequest = make_shared<Alibabacloud_Dingtalkyida_1_0::GetInstanceByIdRequest>(map<string, boost::any>({
  ...
  try {
    client->getInstanceByIdWithOptions(make_shared<string>("f30233fb-72e1-xxx"), getInstanceByIdRequest, getInstanceByIdHeaders, make_shared<Darabonba_Util::RuntimeOptions>(Darabonba_Util::RuntimeOptions()));
  }
