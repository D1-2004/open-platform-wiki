---
title: "发起宜搭审批流程"
source_url: "https://open.dingtalk.com/document/development/initiate-the-approval-process"
namespace: "development"
slug: "initiate-the-approval-process"
group: "应用开发"
tab: "服务端API"
breadcrumb: "历史文档（不推荐） > 宜搭 > 流程 > 发起宜搭审批流程"
doc_id: "X7GH8sEK4N"
updated_at: "2025-09-08 19:03:32"
---

> Source: https://open.dingtalk.com/document/development/initiate-the-approval-process
> Path: 应用开发 / 服务端API / 历史文档（不推荐） > 宜搭 > 流程 > 发起宜搭审批流程
> Updated: 2025-09-08 19:03:32

# 发起宜搭审批流程

调用本接口发起宜搭审批流程到钉钉开放平台。

> **[!IMPORTANT]**
>
> 为提升接口使用体验，针对宜搭部分接口进行升级，本文档预计于 2024 年 11 月 1 日迁移至历史文档（不推荐）目录，接口不再支持新应用接入, 已接入的应用可继续调用，且本接口仅保持现有功能，不再新增支持其他能力。
>
> - 如果未使用本接口，推荐使用[发起宜搭审批流程](https://open.dingtalk.com/document/orgapp/api-startinstance-v2)接口。
> - 如果已使用本接口，建议您根据自身实际情况评估是否切换至推荐接口。

![](https://img.alicdn.com/imgextra/i4/O1CN01rLyTMf1NUTk9O6HZd_!!6000000001573-2-tps-960-288.png)
...
```
POST /v1.0/yida/processes/instances/start HTTP/1.1
...
| processData | String | 否 | 发起人自选节点所选的审批人信息。  **[!NOTE]**     - processData 参数是一个 json 数组，元素个数代表发起人节点个数； - 对于每个发起人自选节点，key 代表发起人自选节点唯一标识，value 代表需要指定的审批人 userId 列表，为数组格式，可以是一个或者多个，以英文逗号隔开。 - 发起人自选节点唯一标识可以从[预览审批流程](https://open.dingtalk.com/document/orgapp/api-previewpublishedprocess)接口的返回值中的 operator 字段中获取。预览含有发起人自选节点的审批流程时，发起人自选节点的 operator 字段会返回一个前缀为\*\*\_*optionalApproval***的唯一标识，例如：**\_\_optionalApproval\_node\_ocltdztr2b3\*\*。 |

## 返回参数

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| result | String | 流程实例ID。 |
...
POST /v1.0/yida/processes/instances/start HTTP/1.1
Host:api.dingtalk.com
x-acs-dingtalk-access-token:EF6Y4xxx
Content-Type:application/json

{
  ...
  "processCode" : "TPROC--EF6Y4xxx",
  "departmentId" : "18295",
  "processData" : "[{ \"key\": \"__optionalApproval_node_ocltdztr2b1\", \t\"value\": [\"5014533041684350\"] }, { \t\"key\": \"__optionalApproval_node_ocltdztr2b3\", \t\"value\": [\"5014533041684350\", \"01536610064226180419\"] }, { \t\"key\": \"__optionalApproval_node_oclte07cwn1\", \t\"value\": [\"01432910392321237660\"] }]"
}
```
...
public class Sample {
    ...
    public static void main(String[] args_) throws Exception {
        ...
        com.aliyun.dingtalkyida_1_0.models.StartInstanceHeaders startInstanceHeaders = new com.aliyun.dingtalkyida_1_0.models.StartInstanceHeaders();
        startInstanceHeaders.xAcsDingtalkAccessToken = "<your access token>";
        com.aliyun.dingtalkyida_1_0.models.StartInstanceRequest startInstanceRequest = new com.aliyun.dingtalkyida_1_0.models.StartInstanceRequest()
                ...
                .setProcessData("[{ \"key\": \"__optionalApproval_node_ocltdztr2b1\", 	\"value\": [\"5014533041684350\"] }, { 	\"key\": \"__optionalApproval_node_ocltdztr2b3\", 	\"value\": [\"5014533041684350\", \"01536610064226180419\"] }, { 	\"key\": \"__optionalApproval_node_oclte07cwn1\", 	\"value\": [\"01432910392321237660\"] }]");
        try {
            client.startInstanceWithOptions(startInstanceRequest, startInstanceHeaders, new com.aliyun.teautil.models.RuntimeOptions());
...
class Sample:
    ...
    def main(
        ...
        start_instance_headers = dingtalkyida__1__0_models.StartInstanceHeaders()
        start_instance_headers.x_acs_dingtalk_access_token = '<your access token>'
        start_instance_request = dingtalkyida__1__0_models.StartInstanceRequest(
            ...
            process_code='TPROC--EF6Y4xxx',
            department_id='18295',
            process_data='[{ "key": "__optionalApproval_node_ocltdztr2b1", 	"value": ["5014533041684350"] }, { 	"key": "__optionalApproval_node_ocltdztr2b3", 	"value": ["5014533041684350", "01536610064226180419"] }, { 	"key": "__optionalApproval_node_oclte07cwn1", 	"value": ["01432910392321237660"] }]'
        )
        try:
            client.start_instance_with_options(start_instance_request, start_instance_headers, util_models.RuntimeOptions())
    ...
    async def main_async(
        ...
        try:
            await client.start_instance_with_options_async(start_instance_request, start_instance_headers, util_models.RuntimeOptions())
        except Exception as err:
            if not UtilClient.empty(err.code) and not UtilClient.empty(err.message):
                # err 中含有 code 和 message 属性，可帮助开发定位问题
                pass
...
class Sample {
    ...
    public static function main($args){
        ...
        $startInstanceHeaders = new StartInstanceHeaders([]);
        $startInstanceHeaders->xAcsDingtalkAccessToken = "<your access token>";
...
func _main (args []*string) (_err error) {
  ...
  startInstanceHeaders := &dingtalkyida_1_0.StartInstanceHeaders{}
  startInstanceHeaders.XAcsDingtalkAccessToken = tea.String("<your access token>")
  startInstanceRequest := &dingtalkyida_1_0.StartInstanceRequest{
    ...
    ProcessCode: tea.String("TPROC--EF6Y4xxx"),
    DepartmentId: tea.String("18295"),
    ProcessData: tea.String("[{ \"key\": \"__optionalApproval_node_ocltdztr2b1\", 	\"value\": [\"5014533041684350\"] }, { 	\"key\": \"__optionalApproval_node_ocltdztr2b3\", 	\"value\": [\"5014533041684350\", \"01536610064226180419\"] }, { 	\"key\": \"__optionalApproval_node_oclte07cwn1\", 	\"value\": [\"01432910392321237660\"] }]"),
  }
  tryErr := func()(_e error) {
    ...
    _, _err = client.StartInstanceWithOptions(startInstanceRequest, startInstanceHeaders, &util.RuntimeOptions{})
    if _err != nil {
      return _err
    }
...
class Client {
  ...
  static async main(args) {
    ...
    let startInstanceHeaders = new dingtalkyida_1_0.StartInstanceHeaders({ });
    startInstanceHeaders.xAcsDingtalkAccessToken = '<your access token>';
    let startInstanceRequest = new dingtalkyida_1_0.StartInstanceRequest({
      appType: 'APP_PBKT0MFBEBTDO8T7SLVP',
      systemToken: 'hexxx',
      userId: '1731234567',
      language: 'zh_CN',
    ...
    try {
      await client.startInstanceWithOptions(startInstanceRequest, startInstanceHeaders, new Util.RuntimeOptions({ }));
    } catch (err) {
...
namespace AlibabaCloud.SDK.Sample
{
    public class Sample 
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AlibabaCloud.SDK.Dingtalkyida_1_0.Models.StartInstanceHeaders startInstanceHeaders = new AlibabaCloud.SDK.Dingtalkyida_1_0.Models.StartInstanceHeaders();
            startInstanceHeaders.XAcsDingtalkAccessToken = "<your access token>";
            AlibabaCloud.SDK.Dingtalkyida_1_0.Models.StartInstanceRequest startInstanceRequest = new AlibabaCloud.SDK.Dingtalkyida_1_0.Models.StartInstanceRequest
            {
                AppType = "APP_PBKT0MFBEBTDO8T7SLVP",
                SystemToken = "hexxx",
                UserId = "1731234567",
            ...
            try
            {
                client.StartInstanceWithOptions(startInstanceRequest, startInstanceHeaders, new AlibabaCloud.TeaUtil.Models.RuntimeOptions());
            }
            catch (TeaException err)
            {
...
| 400 | invalidParameter.parameter.accessKeyNotMatch | 接入秘钥不匹配:%s | 接入秘钥不匹配 |
...
| 400 | failure.operation.applicationKeyIsSensitiveInformation | 应用秘钥为敏感信息， 请联系应用主管理员获取:%s | 应用秘钥为敏感信息， 请联系应用主管理员获取 |
