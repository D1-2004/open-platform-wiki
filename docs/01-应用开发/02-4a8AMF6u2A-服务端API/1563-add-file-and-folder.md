...
> - 如果未使用本接口，推荐使用以下存储接口：
>
>   - 添加文件，推荐使用存储产品[获取文件上传信息](0674-obtain-file-upload-informations.md)和[提交文件](0675-submittal-file.md)接口。
>   - 添加文件夹，推荐使用存储产品[添加文件夹](0654-add-folder.md)接口。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| spaceId | String | 是 | 钉盘空间ID，可调用[获取空间信息](1560-obtain-space-information.md)接口获取spaceId参数值。 |
...
| mediaId | String | 否 | 对应OSS Object Key，可调用[获取文件上传信息](1601-obtain-storage-upload-information.md)接口获取。  **[!NOTE]**  - 当**fileType**值为**file**文件时，此参数必填。 - 当**fileType**值为**folder**文件夹时，此参数可不填。 |
...
| unionId | String | 是 | 用户unionId，可调用[查询用户详情](1457-queries-user-details.md)接口获取unionid参数值。 |
