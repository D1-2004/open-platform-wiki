...
doc_id: "EOtxyB2KRO"
updated_at: "2025-09-12 17:19:01"
---
...
调用通讯录[更新用户信息V1](https://open.dingtalk.com/document/orgapp/update-user-details)接口和[更新用户信息V2](https://open.dingtalk.com/document/orgapp/user-information-update)接口更新员工部门或者员工职位时，接口会出现报错，报错信息如下图所示。如果需要更新员工部门或者员工职位，请调用本接口实现。
...
| positionIdAfterTransfer | String | 否 | 员工调岗后的职位ID。 是否升级职位，该参数填写方式不同。   - 未升级。   如果职位是文本类型，表示未升级，该参数不传。如下图所示。 - 已升级。   该参数值必填，调用[获取企业职位列表](0926-obtain-enterprise-position-information.md)接口获取positionId。有两种方式可升级。    - 方式一：升级职位字段。如下图所示。   - 方式二：升级职位管理。如下图所示。     如果智能人事升级职位管理功能，调用本接口调岗员工的职位，需要检查职位与部门的所属关系。参数deptIdsAfterTransfer内的部门ID必须是该职级设置的下属部门的子集。 |
| positionNameAfterTransfer | String | 否 | 员工调岗后的职位名称，长度最大124字符。  是否升级职位，该参数填写方式不同。   - 未升级。   如果**职位**是文本类型，表示未升级，该参数必填，请填写职位名称。如下图所示。 - 已升级。   无需填写该参数，会自动更新参数职位ID参数positionIdAfterTransfer对应的职位名称，有两种方式可升级。    - 方式一：升级**职位**字段。如下图所示。   - 方式二：升级职位管理。如下图所示。 |
| rankIdAfterTransfer | String | 否 | 员工调岗后的职级ID。 是否升级岗位职级，该参数填写方式不同。   - 未升级。   如果**岗位职级**是文本类型，表示未升级，该参数不传。如下图所示。 - 已升级。   该参数值必填，调用[获取企业职级列表](0927-obtain-enterprise-rank-information.md)接口获取rankId参数值。有两种方式可升级。    - 方式一：升级**岗位职级**字段。如下图所示。   - 方式二：升级职位管理。如下图所示。 |
| positionLevelAfterTransfer | String | 否 | 员工调岗后的职级名称，长度不超过64字符。  是否升级职级，该参数填写方式不同。   - 未升级。   如果**岗位职级**是文本类型，表示未升级，该参数必填，请填写岗位职级名称。如下图所示。 - 已升级。   无需填写该参数，会自动更新参数职级ID参数rankIdAfterTransfer对应的职级名称，有两种方式可升级。    - 方式一：升级**岗位职级**字段。如下图所示。   - 方式二：升级职位管理。如下图所示。 |
| jobIdAfterTransfer | String | 否 | 员工调岗后的职务ID，调用[获取企业职务列表](0928-obtain-enterprise-title-information.md)接口获取jobId参数值。    使用该字段，需要升级职位管理，升级方式如下图所示。 |
