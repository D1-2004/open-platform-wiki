...
slug: "sets-the-visible-range-of-branch-organizations-within-the-group"
group: "应用开发"
tab: "服务端API"
breadcrumb: "通讯录管理 > 上下级组织（原关联组织） > 设置分支组织在主干组织内的可见范围"
doc_id: "64QvR2dfZK"
...
> Path: 应用开发 / 服务端API / 通讯录管理 > 上下级组织（原关联组织） > 设置分支组织在主干组织内的可见范围
> Updated: 2026-05-26 09:01:03

# 设置分支组织在主干组织内的可见范围

调用本接口设置分支组织在主干组织内的可见范围，也可以设置分支组织在主干组织内可以被哪些部门可见。
...
![iShot2022-09-22 09](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8912183661/p494069.png)
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证，通过以下获取：   - 企业内部应用，调用[获取企业内部应用的accessToken](0033-obtain-the-access-token-of-an-internal-app.md)接口获取。 - 第三方企业应用，调用[获取企业accessToken(应用商店应用)](0034-obtain-the-access-token-of-the-authorized-enterprise-1.md)接口获取。   **[!NOTE]**  调用本接口，需要使用**主干组织**的访问凭证，不能使用分支组织的访问凭证。 |
...
| --- | --- | --- | --- |
|  | Array | 是 | 请求参数。 |
| branchCorpId | String | 是 | 伙伴组织的企业corpId，详情参见[基础概念](https://open.dingtalk.com/document/development/development-basic-concepts#section-bbk-mv0-oxd)。 |
| type | Long | 是 | 设置可见性类型，取值：   - **0**：在主干组织通讯录隐藏分支组织。 - **1**：仅可见分支组织自己。 |
| open | Boolean | 是 | 是否开启，取值：   - **true**：开启 - **false**：关闭 |
| visibleBranchCorpIds | Array of String | 否 | 设置例外的加入主干组织的分支企业corpId列表，详情参见[基础概念](https://open.dingtalk.com/document/development/development-basic-concepts#section-bbk-mv0-oxd)。 |
| visibleDeptIds | Array of Long | 否 | 设置例外的部门deptId列表，可调用[获取部门列表](0082-user-management-acquires-the-list-departments.md)接口获取dept\_id参数值。 |
