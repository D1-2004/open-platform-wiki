---
title: "通过高级查询条件获取表单实例数据（包括子表单组件数据）"
source_url: "https://open.dingtalk.com/document/development/query-form-instances-using-advanced-search-conditions"
namespace: "development"
slug: "query-form-instances-using-advanced-search-conditions"
group: "应用开发"
tab: "服务端API"
breadcrumb: "历史文档（不推荐） > 宜搭 > 表单 > 通过高级查询条件获取表单实例数据（包括子表单组件数据）"
doc_id: "8s4gF3RsZI"
updated_at: "2025-09-08 19:03:44"
---

> Source: https://open.dingtalk.com/document/development/query-form-instances-using-advanced-search-conditions
> Path: 应用开发 / 服务端API / 历史文档（不推荐） > 宜搭 > 表单 > 通过高级查询条件获取表单实例数据（包括子表单组件数据）
...
>
> - 如果未使用本接口，推荐使用[通过高级查询条件获取表单实例数据（包括子表单组件数据）](https://open.dingtalk.com/document/orgapp/api-searchformdatasecondgeneration-v2)接口。
> - 如果已使用本接口，建议您根据自身实际情况评估是否切换至推荐接口。

例如，小钉的userId是001，提交人为小钉、实例状态为运行中的表单实例只有1条，如下图所示。调用本接口时，设置相同筛选条件：提交人的userId为001，同时实例状态为运行中，可以获取到符合条件的信息，如实例标题是“小钉发起的报销表单”、提交人是小钉等信息，更多详细字段可以查看参数说明。
![](https://img.alicdn.com/imgextra/i1/O1CN01NNaRYd1wyiSliMcuj_!!6000000006377-2-tps-1536-940.png)
...
```
POST /v1.0/yida/forms/instances/advances/queryAll HTTP/1.1
...
| searchCondition | String | 否 | 用于检索表单实例数据的检索条件。   - 企业内部应用，格式参考[宜搭表单筛选的组件格式说明](https://open.dingtalk.com/document/orgapp/use-the-filter-conditions-in-the-form-data-management-for)。 - 第三方企业应用，格式参考[宜搭表单筛选的组件格式说明](https://open.dingtalk.com/document/isvapp/description-of-component-formats-suitable-for-form-filtering)。 |
...
| orderConfigJson | String | 否 | 排序规则。   - 企业内部应用，详情参考[根据表单组件ID和宜搭内置字段对搜索结果排序](https://open.dingtalk.com/document/orgapp/built-in-fields-that-can-be-used-in-search-results-sorting)。 - 第三方企业应用，详情参考[根据表单组件ID和宜搭内置字段对搜索结果排序](https://open.dingtalk.com/document/isvapp/sort-the-form-by-the-form-component-id-and-by)。 |
...
| sequence | String | 此表单实例所对应的批量导入批次号。  **[!NOTE]**  该表单实例是通过批量导入创建的，会返回该字段。 |
...
| formData | Map | 表单实例数据以Map结构展示。   - 企业内部应用，参考[宜搭表单实例FormData结构说明](https://open.dingtalk.com/document/orgapp/formdata-structure-description-for-returned-form-instances)。 - 第三方企业应用，参考[宜搭表单实例FormData结构说明](https://open.dingtalk.com/document/isvapp/suitable-form-example-formdata-structure-description)。 |
