---
title: "取消企业授权"
source_url: "https://open.dingtalk.com/document/development/e-sign-has-revoked-the-enterprise-authorization"
namespace: "development"
slug: "e-sign-has-revoked-the-enterprise-authorization"
group: "应用开发"
tab: "服务端API"
...
当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[取消企业授权](1063-cancel-enterprise-authorization.md)接口，已接入的开发者结合实际尽快完成迁移。
...
| 字段 | 值 |
| --- | --- |
| HTTP URL | https://api.dingtalk.com/v1.0/esign/corps/auth/cancel |
| HTTP Method | GET |
...
public class Sample {
    ...
    public static void main(String[] args_) throws Exception {
        ...
        try {
            client.cancelCorpAuthWithOptions(cancelCorpAuthHeaders, new RuntimeOptions());
...
class Sample:
    ...
    def main(
        ...
        cancel_corp_auth_headers = dingtalkesign__1__0_models.CancelCorpAuthHeaders()
        cancel_corp_auth_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            client.cancel_corp_auth_with_options(cancel_corp_auth_headers, util_models.RuntimeOptions())
    ...
    async def main_async(
        ...
        cancel_corp_auth_headers.x_acs_dingtalk_access_token = '<your access token>'
        try:
            await client.cancel_corp_auth_with_options_async(cancel_corp_auth_headers, util_models.RuntimeOptions())
        except Exception as err:
            if not UtilClient.empty(err.code) and not UtilClient.empty(err.message):
                # err 中含有 code 和 message 属性，可帮助开发定位问题
                pass
...
func _main (args []*string) (_err error) {
  ...
  cancelCorpAuthHeaders := &dingtalkesign_1_0.CancelCorpAuthHeaders{}
  ...
  tryErr := func()(_e error) {
    ...
    _, _err = client.CancelCorpAuthWithOptions(cancelCorpAuthHeaders, &util.RuntimeOptions{})
    if _err != nil {
      return _err
    }
...
export default class Client {
  ...
  static async main(args: string[]): Promise<void> {
    ...
    let cancelCorpAuthHeaders = new $dingtalkesign_1_0.CancelCorpAuthHeaders({ });
    cancelCorpAuthHeaders.xAcsDingtalkAccessToken = "<your access token>";
    try {
      await client.cancelCorpAuthWithOptions(cancelCorpAuthHeaders, new $Util.RuntimeOptions({ }));
    } catch (err) {
...
namespace AlibabaCloud.SDK.Sample
{
    public class Sample 
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AlibabaCloud.SDK.Dingtalkesign_1_0.Models.CancelCorpAuthHeaders cancelCorpAuthHeaders = new AlibabaCloud.SDK.Dingtalkesign_1_0.Models.CancelCorpAuthHeaders();
            cancelCorpAuthHeaders.XAcsDingtalkAccessToken = "<your access token>";
            try
            {
                client.CancelCorpAuthWithOptions(cancelCorpAuthHeaders, new AlibabaCloud.TeaUtil.Models.RuntimeOptions());
            }
            catch (TeaException err)
            {
...
int main(int argc, char *args[]) {
  ...
  shared_ptr<Alibabacloud_Dingtalkesign_1_0::CancelCorpAuthHeaders> cancelCorpAuthHeaders = make_shared<Alibabacloud_Dingtalkesign_1_0::CancelCorpAuthHeaders>();
  cancelCorpAuthHeaders->xAcsDingtalkAccessToken = make_shared<string>("<your access token>");
  try {
    client->cancelCorpAuthWithOptions(cancelCorpAuthHeaders, make_shared<Darabonba_Util::RuntimeOptions>(Darabonba_Util::RuntimeOptions()));
  }
