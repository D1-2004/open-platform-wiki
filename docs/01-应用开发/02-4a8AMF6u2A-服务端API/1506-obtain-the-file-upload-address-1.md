...
当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[获取文件上传地址](1084-obtain-the-upload-url-of-a-file-1.md)接口，已接入的开发者结合实际尽快完成迁移。
...
| data | Object | 返回结果。 |
| fileId | String | 文件Id。 |
| uploadUrl | String | 文件直传地址。在获取到文件上传地址后，可以直接使用此地址进行文件上传，详情请参考[上传文件](1082-upload-objects-1.md)。      可以重复使用，但是只能传一样的文件，有效期一小时。 |
