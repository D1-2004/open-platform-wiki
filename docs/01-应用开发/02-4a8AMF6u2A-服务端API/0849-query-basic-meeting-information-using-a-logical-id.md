...
> Path: 应用开发 / 服务端API / 专属钉钉 > 根据会议逻辑ID查询会议基本信息
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| logicalConferenceId | String | 是 | 会议逻辑ID，可调用[创建日程](https://open.dingtalk.com/document/development/create-event)接口获取conferenceId参数值。 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| conferenceId | String | 会议实体ID。  **[!NOTE]**    会议正式开始后才会返回该字段。 |
| title | String | 会议标题。 |
