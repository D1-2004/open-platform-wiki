---
title: "知识库权限变更"
source_url: "https://open.dingtalk.com/document/development/change-of-knowledge-base-access-rights"
namespace: "development"
slug: "change-of-knowledge-base-access-rights"
group: "应用开发"
tab: "服务端API"
breadcrumb: "文档/文件 > 知识库 > 使用教程 > 知识库权限变更"
doc_id: "e7jhgL5M5g"
updated_at: "2026-04-08 20:23:51"
---

> Source: https://open.dingtalk.com/document/development/change-of-knowledge-base-access-rights
> Path: 应用开发 / 服务端API / 文档/文件 > 知识库 > 使用教程 > 知识库权限变更
> Updated: 2026-04-08 20:23:51
...
> **[!NOTE]**
>
> 接口请参考[添加权限](0679-add-permissions-file.md)文档说明，如何调用请参考[如何调用服务端API](https://open.dingtalk.com/document/development/how-to-call-apis)文档介绍。

**需要传入：**

- `dentryUuid`（Path）：目标文件/文件夹的唯一标识，通过[搜索文件](0636-search-for-files.md)接口获取；
- `unionId`（Query）：操作者的 unionId，可调用[查询用户详情](0056-query-user-details.md)接口获取；
...
- **解决方案**：需要先对该文件/文件夹调用[设置权限继承模式](0683-set-permission-inheritance-mode.md)接口，将继承模式设为**BREAK**（打断），切断父节点权限的传递，再进行权限修改。

> **[!NOTE]**
>
> 接口请参考[修改权限](0681-modify-permissions-file.md)文档说明，如何调用请参考[如何调用服务端API](https://open.dingtalk.com/document/development/how-to-call-apis)文档介绍。
...
> **[!NOTE]**
>
> 接口请参考[删除权限](0680-delete-permissions-file.md)文档说明，如何调用请参考[如何调用服务端API](https://open.dingtalk.com/document/development/how-to-call-apis)文档介绍。
...
> - 接口请参考[设置权限继承模式](0683-set-permission-inheritance-mode.md)文档说明，如何调用请参考[如何调用服务端API](https://open.dingtalk.com/document/development/how-to-call-apis)文档介绍。
...
> - 接口请参考[获取权限继承模式](0684-get-permission-inheritance-mode.md)文档说明，如何调用请参考[如何调用服务端API](https://open.dingtalk.com/document/development/how-to-call-apis)文档介绍。
...
1. 通过[搜索文件](0636-search-for-files.md)接口，获取目标文件的 `dentryUuid`；
2. 通过[查询用户详情](0056-query-user-details.md)接口，获取目标用户的`userId`和操作者的`unionId`
3. 调用[添加权限](0679-add-permissions-file.md)接口，`roleId = "EDITOR"`，`members[0].type = "USER"`，`members[0].id = 目标用户 userId`。

### 场景二：将某用户的权限从编辑者降级为只读

1. 通过[搜索文件](0636-search-for-files.md)接口，获取目标文件的 `dentryUuid`；
2. 通过[查询用户详情](0056-query-user-details.md)接口，获取目标用户的`userId`；
3. 调用[修改权限](0681-modify-permissions-file.md)接口，`roleId = "READER"`，`members`指定目标用户。

### 场景三：移除某用户对文件的所有权限

1. 通过[搜索文件](0636-search-for-files.md)接口，获取目标文件的 `dentryUuid`；
2. 通过[查询用户详情](0056-query-user-details.md)接口，获取目标用户的`userId`；
3. 调用[删除权限](0680-delete-permissions-file.md)接口，`roleId` 填写该用户当前持有的权限角色，`members` 指定目标用户。

### 场景四：为某个子文件夹设置独立权限（不继承父目录）

1. 通过[搜索文件](0636-search-for-files.md)接口，获取子文件夹的 `dentryUuid`；
2. 调用[设置权限继承模式](0683-set-permission-inheritance-mode.md)接口，`inheritance = "BREAK"`打断权限继承；
3. 再调用[添加权限](0679-add-permissions-file.md)接口，单独为该文件夹配置所需的权限成员和角色。
