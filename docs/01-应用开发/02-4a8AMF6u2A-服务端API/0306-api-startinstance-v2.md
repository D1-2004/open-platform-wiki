...
slug: "api-startinstance-v2"
group: "应用开发"
tab: "服务端API"
breadcrumb: "宜搭 > 流程 > 发起宜搭审批流程"
doc_id: "w2yqFRi6wt"
updated_at: "2026-06-15 10:44:16"
---

> Source: https://open.dingtalk.com/document/development/api-startinstance-v2
> Path: 应用开发 / 服务端API / 宜搭 > 流程 > 发起宜搭审批流程
> Updated: 2026-06-15 10:44:16
...
| **基本信息** | |
| --- | --- |
| HTTP URL | https://api.dingtalk.com/v2.0/yida/processes/instances/start |
| HTTP Method | POST |
...
| processCode | String | 否 | 表单流程编码。，获取方式如下图所示： |
| departmentId | String | 否 | 发起人所在部门ID，可通过[获取部门列表](0082-user-management-acquires-the-list-departments.md)接口获取。 |
| processData | String | 否 | 流程数据。 |
| useAlias | Boolean | 否 | 是否使用组件别名：   - **true**：开启 - **false**：不开启   开启之后，入参formDataJson中组件id支持以别名形式传入。 |
...
```
POST /v2.0/yida/processes/instances/start HTTP/1.1
...
class Sample:
    ...
    def main(
        ...
        start_instance_headers = dingtalkyida__2__0_models.StartInstanceHeaders()
        start_instance_headers.x_acs_dingtalk_access_token = '<your access token>'
        start_instance_request = dingtalkyida__2__0_models.StartInstanceRequest(
...
func _main (args []*string) (_err error) {
  ...
  startInstanceHeaders := &dingtalkyida_2_0.StartInstanceHeaders{}
  startInstanceHeaders.XAcsDingtalkAccessToken = tea.String("<your access token>")
  startInstanceRequest := &dingtalkyida_2_0.StartInstanceRequest{
...
class Client {
  ...
  static async main(args) {
    ...
    let startInstanceHeaders = new dingtalkyida_2_0.StartInstanceHeaders({ });
    startInstanceHeaders.xAcsDingtalkAccessToken = '<your access token>';
    let startInstanceRequest = new dingtalkyida_2_0.StartInstanceRequest({
      appType: 'APP_PBKT0MFBEBTDO8T7SLVP',
      systemToken: 'hexxx',
      userId: '1731234567',
      language: 'zh_CN',
