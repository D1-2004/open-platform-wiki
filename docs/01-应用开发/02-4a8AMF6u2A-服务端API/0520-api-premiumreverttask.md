...
doc_id: "lQWeKp0T9Q"
updated_at: "2025-09-11 21:04:23"
---

> Source: https://open.dingtalk.com/document/development/api-premiumreverttask
> Path: 应用开发 / 服务端API / OA 审批 > 高级版专享接口 > 官方 OA 审批 > 审批实例 > 退回审批任务
> Updated: 2025-09-11 21:04:23
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| taskId | Long | 是 | 审批任务ID，可调用[获取单个审批实例详情](0494-obtains-the-details-of-a-single-approval-instance-pop.md)接口获取。 |
...
| targetActivityId | String | 是 | 退回到的节点ID。   - 可调用[获取审批单流程中的节点信息](0489-approval-process-prediction.md)接口获取审批单流程中的节点ID信息`activityId`参数值。 - 若退回方式为`REVERT_FOR_RESUBMIT` 退回到发起人，则targetActivityId固定传发起节点的ID值：`sid-startevent` |
