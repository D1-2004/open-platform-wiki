...
> Path: 应用开发 / 服务端API / 历史文档（不推荐） > e签宝 1.0 > 签署流程 > 获取发起签署任务地址
...
当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[获取发起签署任务的地址](1079-obtain-the-address-used-to-initiate-a-signed-task.md)接口，已接入的开发者结合实际尽快完成迁移。
