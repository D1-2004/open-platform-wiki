...
> **[!IMPORTANT]**
>
> 当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[删除群成员](0749-api-removescenegroupmember.md)接口，已接入的开发者结合实际尽快完成迁移。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | be311\*\*\*\*\* | 调用该API的应用凭证。   - 企业内部应用，通过[获取企业内部应用的access\_token](1420-obtain-orgapp-token.md)接口获取。 - 第三方企业应用，通过[获取第三方企业的access\_token](1422-obtain-isvapp-token.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| open\_conversation\_id | String | 是 | cid15\*\*\*\*\*== | 群ID，调用[创建群](1461-create-a-scene-group-v2.md)接口获取`open_conversation_id`参数值。 |
...
| 400020 | 没有访问权限 | 请在调用[创建群](1461-create-a-scene-group-v2.md)接口后，延迟几秒再调用本接口 |
