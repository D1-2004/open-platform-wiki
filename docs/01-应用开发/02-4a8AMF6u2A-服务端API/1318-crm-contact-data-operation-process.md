...
doc_id: "qd944mJZCf"
updated_at: "2025-09-23 19:25:41"
---
...
1. 调用服务端API-[获取联系人的元数据](https://open.dingtalk.com/document/orgapp/gets-the-metadata-description-of-a-crm-contact-object)接口，获取客户管理联系人元数据信息。
2. 调用服务端API-[批量新增联系人数据](https://open.dingtalk.com/document/orgapp/add-contact-data-in-batches)接口，进行联系人数据的创建，获取联系人数据`instanceId`（部分接口可能定义为relationId或data\_id，含义相同）。
3. 根据联系人数据`instanceId`进行联系人数据管理操作。

   - 根据联系人数据`instanceId`，调用服务端API-[批量修改联系人数据](https://open.dingtalk.com/document/orgapp/modify-contact-data-in-batches)接口，进行更新联系人数据。
   - 根据联系人数据`instanceId`，调用服务端API-[删除联系人数据](https://open.dingtalk.com/document/orgapp/delete-crm-contact)接口，进行删除联系人数据。
   - 根据联系人数据`instanceId`，调用服务端API-[按照ID列表批量获取联系人数据](https://open.dingtalk.com/document/orgapp/retrieves-contact-data-in-batches-based-on-the-id-list)接口，进行批量获取联系人数据信息。
4. 调用服务端API-[根据指定条件查询联系人数据](https://open.dingtalk.com/document/orgapp/dingtalk-the-contact-data-query-api)接口，查询符合指定条件的联系人数据。
...
2. 填写应用的基本信息，然后单击**确定创建**。
   ...
   ![2](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6523109661/p522020.png)
...
1. 调用服务端API-[获取联系人的元数据](https://open.dingtalk.com/document/orgapp/gets-the-metadata-description-of-a-crm-contact-object)接口，获取客户管理联系人元数据信息。
   ...
   > **[!NOTE]**
   >
   > 本接口使用服务端API接口，SDK下载详情参见[服务端SDK下载](https://open.dingtalk.com/document/orgapp/download-the-server-side-sdk)。
...
2. 调用服务端API-[批量新增联系人数据](https://open.dingtalk.com/document/orgapp/add-contact-data-in-batches)接口，进行联系人数据的创建，获取联系人数据`instanceId`（部分接口可能定义为relationId或data\_id，含义相同）。
   ...
   > **[!NOTE]**
   >
   > 本接口使用新服务端API接口，SDK下载详情参见[新版服务端SDK下载](https://open.dingtalk.com/document/orgapp/sdk-download)。
...
3. 根据联系人数据`instanceId`进行联系人数据管理操作。
   ...
   - 根据联系人数据instanceId，调用服务端API-[批量修改联系人数据](https://open.dingtalk.com/document/orgapp/modify-contact-data-in-batches)接口，进行更新联系人数据。
