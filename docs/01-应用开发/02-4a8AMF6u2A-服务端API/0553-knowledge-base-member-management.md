---
title: "知识库成员管理"
source_url: "https://open.dingtalk.com/document/development/knowledge-base-member-management"
namespace: "development"
slug: "knowledge-base-member-management"
group: "应用开发"
tab: "服务端API"
breadcrumb: "文档/文件 > 知识库 > 使用教程 > 知识库成员管理"
doc_id: "Wr4um2g2Xi"
updated_at: "2026-04-08 20:23:06"
---

> Source: https://open.dingtalk.com/document/development/knowledge-base-member-management
> Path: 应用开发 / 服务端API / 文档/文件 > 知识库 > 使用教程 > 知识库成员管理
> Updated: 2026-04-08 20:23:06
...
1. 调用[获取知识库列表](0557-get-knowledge-base-list.md)接口，从返回结果中获取目标知识库的`rootNodeId`字段，即知识库根节点的`dentryUuid`。
2. 获取成员 ID：
   ...
   - 添加**群会话**：使用群的`chatId`。
3. 调用[添加权限](0679-add-permissions-file.md)接口，根据业务需要选择合适的角色（如协作编辑选 **EDITOR**，只读访问选 **READER**），将成员加入知识库。
...
1. 如需确认成员当前角色，可先调用[获取权限列表](0682-get-permission-list.md)接口查看。
2. 调用[修改权限](0681-modify-permissions-file.md)接口，指定成员和目标角色，完成角色变更。
...
1. 确认成员当前持有的角色，调用[获取权限列表](0682-get-permission-list.md)接口，获取该成员当前的角色（`roleId`），移除时需与实际持有角色一致。
2. 调用[删除权限](0680-delete-permissions-file.md)接口。
...
调用[添加权限](0679-add-permissions-file.md)接口，将成员类型设为 **ORG**，并指定对应的公开角色。
...
调用[获取权限列表](0682-get-permission-list.md)接口，支持按角色过滤，结果支持分页。
