...
slug: "create-multiple-form-instances"
group: "应用开发"
tab: "服务端API"
breadcrumb: "宜搭 > 表单 > 批量创建表单实例"
doc_id: "J28uXSUqBQ"
updated_at: "2026-04-24 14:14:24"
---

> Source: https://open.dingtalk.com/document/development/create-multiple-form-instances
> Path: 应用开发 / 服务端API / 宜搭 > 表单 > 批量创建表单实例
> Updated: 2026-04-24 14:14:24
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| noExecuteExpression | Boolean | 否 | 是否不触发表单绑定的校验规则、关联业务规则和第三方服务回调。   - **true**：不触发 - **false**：触发     该参数值传true可以降低API的耗时以及获得更大的单次保存数据量上限。 |
...
| keepRunningAfterException | Boolean | 否 | 批量保存多条数据异常时，是否跳过异常继续保存下一条数据。   - **true**：跳过 - **false**：不跳过     当noExecuteExpression为false时此参数才生效。 |
| userId | String | 是 | 用户userId，可通过[获取部门用户userid列表](0065-query-the-list-of-department-userids.md)接口获取。 |
| formDataJsonList | Array of String | 是 | 表单实例数据，格式参考[创建或更新表单数据格式说明](0363-instructions-for-creating-or-updating-form-data-format.md)。 |
