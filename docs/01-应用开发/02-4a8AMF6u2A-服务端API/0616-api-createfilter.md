...
slug: "api-createfilter"
group: "应用开发"
tab: "服务端API"
breadcrumb: "文档/文件 > 表格 > 筛选 > 创建筛选"
doc_id: "MHwqLUgyYj"
updated_at: "2026-04-17 15:00:47"
---

> Source: https://open.dingtalk.com/document/development/api-createfilter
> Path: 应用开发 / 服务端API / 文档/文件 > 表格 > 筛选 > 创建筛选
> Updated: 2026-04-17 15:00:47

# 创建筛选

调用本接口可在钉钉表格的指定工作表上创建全局筛选，并指定筛选范围。适用于需要对表格数据进行行级过滤展示的场景，例如在销售报表中仅显示特定区域或状态的数据。

## **接口调用说明**

- 接口调用成功后，可通过设置筛选条件接口为各列配置筛选规则，不满足条件的行将被自动隐藏。
...
| 字段 | 值 |
| --- | --- |
| HTTP URL | https://api.dingtalk.com/v1.0/doc/workbooks/{workbookId}/sheets/{sheetId}/createFilter |
| HTTP Method | POST |
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| range | String | 是 | 筛选范围，使用 A1 表示法。例如 `A1:D10` 表示从 A1 到 D10 的区域。筛选范围的第一行将作为标题行，不参与筛选计算。 |
| criteria | Map<String, Object> | 否 | 可选。创建筛选时同时设置各列的筛选条件。key 为列偏移量（相对于筛选范围首列，从 0 开始的字符串），value 为该列的筛选条件对象（FilterCriteria）。不传则创建空筛选。 |
|  | Object | 否 | 当前列的筛选条件 |
