...
group: "应用开发"
tab: "服务端API"
breadcrumb: "OA 审批 > 使用教程 > 审批页面托管集成模式 > 审批操作区托管接口协议详解"
doc_id: "yUlBMf9JNL"
updated_at: "2025-09-10 19:31:53"
---
...
> Path: 应用开发 / 服务端API / OA 审批 > 使用教程 > 审批页面托管集成模式 > 审批操作区托管接口协议详解
> Updated: 2025-09-10 19:31:53
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/4087165271/p844719.png)
...
| ∟schema | json Array  [{  componentName: 'TextareaField',  **props: {**  **id: 'comment',**  **},**  }] | 弹窗中的表单定义组件列表（behavior为modal类型时需配置该值） | 参考[保存流程中心外部集成审批模板](0538-api-premiumsaveintegratedprocess.md)中的【**FormComponent参数补充说明**】   - **TextField**：单行输入框 - **TextareaField**：多行输入框 - **DDSelectField**：单选框 - **DDMultiSelectField**：多选框 - **TextNote**：文字说明控件 - **DDPhotoField**：图片控件 - **DDAttachment**：附件 - **InnerContactField**：联系人控件    - users string[] staffId列表 |
...
| ∟value | string | 控件值 | 参考[保存流程中心外部集成审批模板](0538-api-premiumsaveintegratedprocess.md)中的示例 |
...
参考[创建或更新审批模板](https://open.dingtalk.com/document/development/create-or-update-approval-templates-new)中的【**FormComponent参数补充说明**】，当前支持以下表单组件类型：
...
**同步审批模板数据到钉钉****：**调用新版服务端API-[保存流程中心外部集成审批模板](0538-api-premiumsaveintegratedprocess.md)，获取模板的唯一编码`processCode`。**注意：该场景需对processFeatureConfig中的CUSTOM\_ACTION\_DEFINITION（表示获取操作区按钮数据的回调地址，即按钮渲染回调）、CUSTOM\_ACTION\_APPLY（表示进行审批操作时回调的回调地址，即操作审批回调）模块进行自定义配置。**
...
**同步审批实例数据到钉钉****：**根据模板编码`processCode`，调用新版服务端API-[保存流程中心外部集成审批实例](https://open.dingtalk.com/document/development/api-premiumsaveintegratedprocessinstance)发起审批实例，获取审批实例`processInstanceId`。**注意：若在创建审批模板时未注册自定义审批操作按钮相关配置，也支持在创建实例维度进行指定，若同时在模板/实例维度进行了配置，优先以实例维度为准。**
...
**同步审批任务数据到钉钉：**根据审批实例`processInstanceId`和待办事项列表tasks，调用[保存流程中心外部集成审批任务](0540-api-premiumsaveintegratedtask.md)，可以将三方系统内的审批节点信息同步到钉钉OA审批，获取待办事项的taskId并生成对应的钉钉待办任务。
