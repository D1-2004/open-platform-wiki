...
slug: "obtain-file-upload-informations"
group: "应用开发"
tab: "服务端API"
...
> **[!NOTE]**
>
> - 如果上传文件时提供的入参为spaceId字段，使用[获取文件上传信息](1578-obtain-storage-upload-information.md)接口。
...
步骤三：调用[提交文件](0673-submittal-file.md)接口，完成文件上传。

> **[!NOTE]**
>
> - 存储空间类型为USER时，空间拥有权限的用户都有操作权限，其他员工调用[添加权限](0679-add-permissions-file.md)接口进行授权。
> - 存储空间类型为APP时，任何人操作都需要调用[添加权限](0679-add-permissions-file.md)接口进行授权。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| parentDentryUuid | String | 是 | 父节点dentryUuid，可调用[搜索文件](0636-search-for-files.md)或[获取 dentryUuid 信息](0670-api-getuuidbydentryid.md)接口，获取返回参数`dentryUuid`字段。    如果是空间根目录, 填空间根目录的dentryUuid。 |
...
| storageDriver | String | 否 | 文件存储驱动类型。    当前只支持DINGTALK。 |
| preCheckParam | Object | 否 | 预检查的字段。可实现对文件名称，文件完整性，容量的校验 |
| size | Long | 否 | 文件大小, 做容量相关校验。不传则不做校验。 |
| name | String | 否 | 文件名称, 文件名称合法性和文件名称冲突校验。    规则：   - 头尾不能包含空格，否则会自动去除。 - 不能包含特殊字符，包括：制表符、\*、"、<、>、|。 - 不能以"."结尾。 |
...
| headerSignatureInfo | Object | Header加签上传信息。    当protocol参数传HEADER\_SIGNATURE时，返回该字段。 |
...
| internalResourceUrls | Array of String | 内网地址。    该字段目前无使用场景，请忽略。 |
