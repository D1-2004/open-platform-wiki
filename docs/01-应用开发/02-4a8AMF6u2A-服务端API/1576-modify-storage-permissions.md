...
doc_id: "UWVq1YSI1V"
updated_at: "2025-09-08 19:07:12"
---
...
>
>    - 如果未使用本接口，推荐使用[修改权限](https://open.dingtalk.com/document/orgapp/modify-permissions-file)接口。
>    - 如果已使用本接口，建议您根据自身实际情况评估是否切换至推荐接口。
> 2. 新老接口中的ID不兼容, 不支持混用。

例如，员工小钉在测试空间内拥有EDITOR编辑权限。调用本接口可更新小钉在测试空间的权限，可以将小钉的权限修改为READER查看权限。
...
| --- | --- | --- | --- |
| 企业内部应用 | 支持 | 企业存储授权信息写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=storage_1.0%23UpdatePermission) |
| 第三方企业应用 | 支持 | 企业存储授权信息写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=storage_1.0%23UpdatePermission) |
...
| dentryId | String | 是 | 文件或文件夹id。   - 企业内部应用，调用[获取文件或文件夹列表](https://open.dingtalk.com/document/orgapp/obtain-the-file-list-storage)接口获取id参数值。 - 第三方企业应用，调用[获取文件或文件夹列表](https://open.dingtalk.com/document/isvapp/obtain-the-file-list-storage)接口获取id参数值。 |
