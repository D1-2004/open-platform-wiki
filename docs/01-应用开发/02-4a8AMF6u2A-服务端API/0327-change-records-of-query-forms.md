...
slug: "change-records-of-query-forms"
group: "应用开发"
tab: "服务端API"
...
调用本接口，可以批量查询同一个应用、同一个表单（包括普通表单和流程表单）下多个实例的全部变更记录。变更记录内容包含：操作人、操作时间、操作字段、操作类型（新增、修改或删除）。
...
| 字段 | 值 |
| --- | --- |
| HTTP URL | https://api.dingtalk.com/v1.0/yida/forms/operationsLogs/query |
| HTTP Method | POST |
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| formUuid | String | 是 | 表单编码。      该参数从宜搭应用中获取。 |
...
```
POST /v1.0/yida/forms/operationsLogs/query HTTP/1.1
