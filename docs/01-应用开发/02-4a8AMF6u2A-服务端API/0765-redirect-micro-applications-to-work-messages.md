...
doc_id: "jdm4dmQlJ5"
updated_at: "2025-10-21 14:06:00"
---
...
4. 获取应用访问凭证[获取企业内部应用的access\_token](1420-obtain-orgapp-token.md)。调用接口时，通过accessToken鉴权调用者身份。
5. 调用[发送工作通知](0767-asynchronous-sending-of-enterprise-session-messages.md)接口，给目标用户发送工作消息通知，需要用户点击消息进行跳转，所以适合的消息类型有link消息、OA消息和卡片消息，具体可参考[消息通知类型](0773-message-types-and-data-format.md)。

   > 第三方企业应用，需要先构建消息模板后才能发送工作通知，详情参考[管理消息模板](0763-manage-message-templates.md)。

## 点击工作通知跳转网页应用（H5 微应用）

1. 调用[发送工作通知](0767-asynchronous-sending-of-enterprise-session-messages.md)接口，工作消息的格式选择link消息、OA消息或者卡片消息，这三种类型消息可以进行跳转。
2. 跳转链接的构造参考[消息链接说明](0774-message-link-description.md)，如下提供这三种类型消息中链接的构造示例，完整的消息发送格式请参考[消息通知类型](0773-message-types-and-data-format.md)。
   ...
   | app\_id | - 企业内部应用-H5微应用，该参数填写0\_agentId，由数字0、下划线和agentId拼接组成，agentId查看请参考[AgentId](https://open.dingtalk.com/document/development/development-basic-concepts#884d363067bnq)。 - 第三方企业应用-H5微应用，该参数填写当前三方微应用的appId值，appId查看请参考[App ID](https://open.dingtalk.com/document/development/development-basic-concepts#f20a795ad844u)。 |
