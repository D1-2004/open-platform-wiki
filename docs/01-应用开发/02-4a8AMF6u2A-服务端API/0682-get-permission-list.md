...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| dentryUuid | String | 是 | 文件uuid，可调用[搜索文件](0636-search-for-files.md)接口或[获取dentryUuid信息](0670-api-getuuidbydentryid.md)接口，获取返回参数dentryUuid字段。 |
...
| member | Object | 权限成员。 |
| type | String | 权限成员类型，枚举值：   - **ORG**：企业 - **DEPT**：部门 - **TAG**：自定义tag - **CONVERSATION**：会话 - **USER**：用户 |
| id | String | 权限成员id：   - `type=ORG`时，id为企业id - `type=DEPT`时，id为部门id - `type=TAG`时，id为标签id - `type=CONVERSATION`时，id为会话id - `type=USER`时，id为员工userId |
