...
| **资源名+path定义** | **字段** | **必填** | **说明** |
| --- | --- | --- | --- |
| action/open\_mini\_app | miniAppId | 是 | 小程序miniAppId。   - 企业内部应用，可参考[基础概念-miniAppId](https://open.dingtalk.com/document/orgapp/basic-concepts)。 - 第三方企业应用，可参考[基础概念-miniAppId](https://open.dingtalk.com/document/isvapp/basic-concepts)。 |
| appId | 否 | 小程序appId。  **[!IMPORTANT]**  第三方企业小程序，该参数必传，可参考[基础概念-appId](https://open.dingtalk.com/document/isvapp/basic-concepts#title-5hv-wew-n60)。 |
| type | 是 | 小程序类型。   - **1**：三方企业应用 - **2**：企业内部应用 - **4**：第三方个人应用 |
| page | 否 | 访问小程序应用的具体某个页面的路径，不设置则打开首页，需要encode处理。  例如：`%2Fpages%2Fmain%2Fmain`。 |
| target | 否 | 打开的容器类型：   - 移动端：    - **fullScreen**（默认）：全屏容器打开   - **panel**：沉浸式容器打开  **[!NOTE]**  什么是沉浸式容器，请参考[沉浸式容器](https://open.dingtalk.com/document/isvapp/immersive-container)。 - 桌面端：    - **slide**（默认），侧边栏打开   - **panel**：第四栏打开   - **popupWindow**：独立弹窗打开 |
...
| agentId | 否 | 小程序应用的agentId。   - 企业内部应用，可参考[基础概念-agentId](https://open.dingtalk.com/document/orgapp/basic-concepts)。 - 第三方企业应用，可参考[基础概念-agentId](https://open.dingtalk.com/document/isvapp/basic-concepts)。   **[!NOTE]**  如果是企业内部小程序，该字段必填。 |
