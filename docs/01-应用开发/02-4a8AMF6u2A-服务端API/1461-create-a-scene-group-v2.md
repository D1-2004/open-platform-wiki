...
slug: "create-a-scene-group-v2"
group: "应用开发"
tab: "服务端API"
breadcrumb: "历史文档（不推荐） > 即时通信 > 会话管理 > 场景群 > 创建群"
doc_id: "Rd9vXSwYHS"
updated_at: "2026-05-13 15:50:33"
---

> Source: https://open.dingtalk.com/document/development/create-a-scene-group-v2
> Path: 应用开发 / 服务端API / 历史文档（不推荐） > 即时通信 > 会话管理 > 场景群 > 创建群
> Updated: 2026-05-13 15:50:33

# 创建群

调用本接口根据群模板ID创建群，适用于企业需要根据群模板快速创建群聊的场景，如项目协作、活动组织等。

> **[!IMPORTANT]**
>
> 当前接口已完成升级迭代且不再支持新应用申请，存量应用调用不受影响，建议未接入的开发者使用[创建场景群](0744-create-a-scene-group.md)接口，已接入的开发者结合实际尽快完成迁移。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | beE4\*\*\*\*\*75d | 调用该API的应用凭证。   - 企业内部应用，通过[获取企业内部应用的access\_token](1420-obtain-orgapp-token.md)接口获取。 - 第三方企业应用，通过[获取第三方企业的access\_token](1422-obtain-isvapp-token.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| title | String | 是 | 测试群 | 群名称。  **[!NOTE]**  最长不超过30字符，建议长度在10字符以内。 |
...
| user\_ids | String | 否 | 072\*\*\*\*\*,013\*\*\*\*\* | 群成员userid列表。  **[!NOTE]**  最多传999个。 |
| subadmin\_ids | String | 否 | 072\*\*\*\*\*,013\*\*\*\*\* | 群管理员userid列表。 |
| uuid | String | 否 | axcf\*-\*\*\*\*\*-\*\*\*\*\*-23da\* | 建群去重的业务ID，由接口调用方指定。  **[!NOTE]**  建议长度在64字符以内。 |
| icon | String | 否 | @lADOADma\*\*\*\*\*QKA | 群头像，格式为`mediaId`。需要调用[上传媒体文件](0644-upload-media-files.md)接口上传群头像，获取mediaId。 |
...
| chat\_id | String | chat6d99a92e8x\*\*\* | 群ID。  **[!NOTE]**  老版本群ID，后续会逐步废弃此参数。 |
