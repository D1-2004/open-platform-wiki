...
| **扩展点** | **描述** |
| --- | --- |
| [登录流程扩展（Android）](0026-login-process-extension-android-1.md) | 主要用于内访外的场景，需要在登录前连接VPN |
| [首页生命周期扩展（Android）](0027-home-lifecycle-extension-android-1.md) | 主要用于外访内的场景，唤起钉钉后自动连接VPN |
| [设置项扩展点（Android）](0023-set-item-extension-point-android-1.md) | 可用于添加VPN相关的用户设置 |
...
| **开放API** | **描述** |
| --- | --- |
| [获取员工信息](0035-get-employee-information.md) | 主要用于校验用户身份的场景 |
...
- 方式1：
  ...
  额外开发一个H5/小程序应用，用户可以在该应用中开启/断开VPN连接，可以参考[开发自定义JSAPI](0007-define-extension-jsapi.md)自定义JSAPI实现。
- 方式2：

  在设置项中新增VPN设置页，用户可以在页面中开启/断开VPN连接。方案较简单，可以参考[设置项扩展点（Android）](0023-set-item-extension-point-android-1.md)自定义Activity实现。
- 方式3（推荐）：
...
在VPN连接时，还可以进一步使用开放API（[获取员工信息](0035-get-employee-information.md)）做账号的入网管控，即获取用户员工ID（可映射成工号），根据员工身份获取相应的入网策略。
...
- 使用[登录流程扩展（Android）](0026-login-process-extension-android-1.md)，在账号登录前连接VPN通道
...
可使用[设置项扩展点（Android）](0023-set-item-extension-point-android-1.md)，添加网络设置页面，完善在网络异常时自检测等功能。
